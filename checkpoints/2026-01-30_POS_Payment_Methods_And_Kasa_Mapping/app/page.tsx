  ...
  const handleFinalize = async () => {
    ...
    try {
      const success = await processSale({
        ...
        installmentLabel: installmentLabel, // Pass the exact label
    ...
    <div style={{
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '12px', minWidth: 0 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px' }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', padding: '12px', borderRadius: '12px', cursor: 'pointer', position: 'relative' }} onClick={() => suspendedSales.length > 0 && setShowResumptionModal(true)}>
            <div style={{ fontSize: '9px', fontWeight: '900', color: suspendedSales.length > 0 ? '#ff8a00' : 'rgba(255,255,255,0.4)', letterSpacing: '1px' }}>⏳ BEKLEYENLER</div>
            <div style={{ fontSize: '18px', fontWeight: '800', marginTop: '4px' }}>{suspendedSales.length} <span style={{ fontSize: '11px', opacity: 0.5 }}>Sepet</span></div>
            {suspendedSales.length > 0 && (
              <div style={{ position: 'absolute', top: '-5px', right: '-5px', background: '#ff8a00', color: 'white', fontSize: '10px', width: '20px', height: '20px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold' }}>{suspendedSales.length}</div>
            )}
          </div>
        </div>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', opacity: 0.05 }}>
              ...
              {suspendedSales.length > 0 && (
                <button
                  ...
                  style={{ marginTop: '20px', padding: '10px 20px', borderRadius: '8px', background: 'rgba(255, 138, 0, 0.1)', border: '1px solid rgba(255, 138, 0, 0.3)', color: '#ff8a00', fontWeight: 'bold', fontSize: '12px', cursor: 'pointer', pointerEvents: 'auto', opacity: 1 }}
                >
                  ASKIDAKİ SEPETLERİ GÖR ({suspendedSales.length})
                </button>
              )}
            </div>
          ) : (
                ...
                {[
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      // Auto-select linked kasa if available
                      if (m.linkedKasaId) {
                        setSelectedKasa(m.linkedKasaId);
                      } else {
                        // Fallback Logic if no specific link
                    ...
                    style={{
                      padding: '12px 8px',
                      padding: '12px 8px',
                      background: paymentMode === m.type ? (m.type === 'account' ? '#F50057' : '#FF5500') : 'rgba(255,255,255,0.03)',
                      borderRadius: '8px', border: paymentMode === m.type ? '1px solid white' : '1px solid rgba(255,255,255,0.05)',
                      cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px', transition: '0.2s'
                    }}
                  >
                    <span style={{ fontSize: '22px' }}>{m.icon}</span>
                    <span style={{ color: 'white', fontSize: '10px', fontWeight: 'bold', opacity: paymentMode === m.type ? 1 : 0.5 }}>{m.label}</span>
                  </button>
                ))}
              </div>
            ...
            {cart.length > 0 && (
              <button
                ...
                style={{
                  ...
                  color: 'rgba(255,255,255,0.7)', fontSize: '12px', fontWeight: 'bold', border: '1px solid rgba(255,255,255,0.1)',
                  cursor: 'pointer', transition: '0.2s'
                }}
      ...
      <style jsx>{`
        ...
        .suspend-row-hover:hover { background: rgba(255, 138, 0, 0.05) !important; border-color: #ff8a00 !important; }
        @keyframes ticker {
      ...
      {showSuspendModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(10px)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#0a0a12', width: '400px', borderRadius: '24px', border: '1px solid rgba(255,255,255,0.1)', padding: '30px' }}>
            <h2 style={{ margin: '0 0 10px 0', fontSize: '18px', fontWeight: '900' }}>SEPETİ ASKIYA AL</h2>
            <p style={{ margin: '0 0 20px 0', fontSize: '12px', color: 'rgba(255,255,255,0.5)' }}>Bu sepeti daha sonra geri yüklemek için bir tanım (plaka, isim vb.) girin.</p>
            <input
              autoFocus
              type="text"
              placeholder="Örn: 34 ABC 123 veya Kırmızı Montlu"
              value={suspenseLabel}
              onChange={e => setSuspenseLabel(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && handleSuspendSale()}
              style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', padding: '15px', borderRadius: '12px', color: 'white', fontSize: '16px', fontWeight: 'bold', outline: 'none', marginBottom: '20px' }}
            />
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setShowSuspendModal(false)} style={{ flex: 1, padding: '12px', borderRadius: '10px', background: 'rgba(255,255,255,0.05)', border: 'none', color: 'white', fontWeight: 'bold', cursor: 'pointer' }}>VAZGEÇ</button>
              <button onClick={handleSuspendSale} style={{ flex: 1, padding: '12px', borderRadius: '10px', background: '#FF5500', border: 'none', color: 'white', fontWeight: 'bold', cursor: 'pointer' }}>ASKIYA AL</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showResumptionModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(10px)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#0a0a12', width: '500px', borderRadius: '24px', border: '1px solid rgba(255,255,255,0.1)', padding: '30px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h2 style={{ margin: 0, fontSize: '18px', fontWeight: '900' }}>ASKIDAKİ SEPETLER</h2>
              <button onClick={() => setShowResumptionModal(false)} style={{ background: 'none', border: 'none', color: '#fff', fontSize: '28px', cursor: 'pointer' }}>×</button>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '400px', overflowY: 'auto' }}>
              {suspendedSales.map((sale) => (
                <div key={sale.id} onClick={() => handleResumeSale(sale.id)} style={{ padding: '20px', background: 'rgba(255,255,255,0.03)', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.1)', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }} className="suspend-row-hover">
                  <div>
                    <div style={{ fontWeight: '900', fontSize: '16px', color: '#ff8a00' }}>{sale.label}</div>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontWeight: '900', fontSize: '18px' }}>₺{sale.total.toLocaleString()}</div>
                    <div style={{ fontSize: '10px', color: '#00e676', fontWeight: 'bold' }}>GERİ YÜKLE ➔</div>
                  </div>
                </div>
              ))}
            </div>
