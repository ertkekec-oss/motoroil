...
export default function SettingsPage() {
    ...
    const { branches: contextBranches, refreshBranches } = useApp();
    ...
    // --- 3. TANIMLAR STATE ---
    const [brands, setBrands] = useState(['Yamaha', 'Honda', 'Motul', 'Shimano', 'Carraro']);
    const [custClasses, setCustClasses] = useState(['Bireysel', 'Kurumsal', 'VIP', 'Kara Liste']);
    const [prodCats, setProdCats] = useState(['Motor Yağı', 'Filtre', 'Lastik', 'Aksesuar', 'Hizmet']);
    const [warranties, setWarranties] = useState(['6 Ay', '1 Yıl', '2 Yıl', '3 Yıl', '5 Yıl']);

    const [newItemInput, setNewItemInput] = useState('');

    const addDefinition = (list: string[], setList: any) => {
        if (!newItemInput) return;
        setList([...list, newItemInput]);
        setNewItemInput('');
    };

    const removeDefinition = (item: string, list: string[], setList: any) => {
                            ...
                            <div className="flex-col gap-4">
                                ...
                                <div className="flex-between" style={{ padding: '16px', background: 'var(--bg-deep)', borderRadius: '12px', border: '1px solid var(--border-light)' }}>
                                    ...
                                    <div className="flex-center gap-2">
                                        <input
                                            type="number"
                                            value={useApp().serviceSettings.motoMaintenancePrice}
                                            onChange={(e) => useApp().updateServiceSettings({ ...useApp().serviceSettings, motoMaintenancePrice: Number(e.target.value) })}
                                ...
                                <div className="flex-between" style={{ padding: '16px', background: 'var(--bg-deep)', borderRadius: '12px', border: '1px solid var(--border-light)' }}>
                                    ...
                                    <div className="flex-center gap-2">
                                        <input
                                            type="number"
                                            value={useApp().serviceSettings.bikeMaintenancePrice}
                                            onChange={(e) => useApp().updateServiceSettings({ ...useApp().serviceSettings, bikeMaintenancePrice: Number(e.target.value) })}
                                            style={{ width: '100px', padding: '10px', borderRadius: '8px', background: 'var(--bg-card)', border: '1px solid var(--primary)', color: 'white', fontWeight: 'bold', textAlign: 'right' }}
                                        />
                                        <span style={{ fontWeight: 'bold' }}>₺</span>
                                    </div>
                        ...
                        <div className="card glass" style={{ maxWidth: '500px' }}>
                            ...
                            <div className="flex-col gap-2 mb-4">
                                <input
                                    ...
                                    onKeyDown={e => {
                                        if (e.key === 'Enter') {
                                            if (definitionTab === 'brands') addDefinition(brands, setBrands);
                                            if (definitionTab === 'prod_cat') addDefinition(prodCats, setProdCats);
                                            if (definitionTab === 'cust_class') addDefinition(custClasses, setCustClasses);
                                            if (definitionTab === 'warranties') addDefinition(warranties, setWarranties);
                                        }
                                    }}
                            ...
                            <div className="flex-col gap-2">
                                {(definitionTab === 'brands' ? brands : definitionTab === 'prod_cat' ? prodCats : definitionTab === 'warranties' ? warranties : custClasses).map((item, i) => (
                                    <div key={i} className="flex-between" style={{ padding: '10px', background: 'rgba(255,255,255,0.05)', borderRadius: '6px' }}>
                                        <span>{item}</span>
                                        <button
                                            onClick={() => {
                                                if (definitionTab === 'brands') removeDefinition(item, brands, setBrands);
                                                if (definitionTab === 'prod_cat') removeDefinition(item, prodCats, setProdCats);
                                                if (definitionTab === 'cust_class') removeDefinition(item, custClasses, setCustClasses);
                                                if (definitionTab === 'warranties') removeDefinition(item, warranties, setWarranties);
                                            }}
                    ...
                    <div style={{ maxWidth: '700px' }}>
                        ...
                        <div className="card glass flex-col gap-6">
                            <div className="flex-between" style={{ borderBottom: '1px solid var(--border-light)', paddingBottom: '16px' }}>
                                ...
                                <button onClick={() => showSuccess('Yedekleme Başlatıldı', '📥 Sistem yedeği oluşturuluyor...')} className="btn btn-outline">⬇️ Hemen İndir</button>
                            </div>
                ...
                {activeTab === 'notifications' && (
                    <div style={{ maxWidth: '600px' }}>
                        ...
                        <div className="card glass flex-col gap-6">
                            ...
                            <button className="btn btn-primary w-full" onClick={() => showSuccess('Başarılı', '✅ Bildirim tercihleri kaydedildi.')}>Ayarları Kaydet</button>
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'logs' && (
                    <div>
                        <div className="flex-between mb-6">
                            ...
                            <button className="btn btn-outline" onClick={() => window.location.reload()}>🔄 Yenile</button>
                        </div>
                        ...
                        <div className="card">
                            <table style={{ width: '100%', textAlign: 'left', borderCollapse: 'collapse' }}>
                                ...
                                <tbody style={{ fontSize: '13px' }}>
                                    {[
                                        { id: 1, date: '2026-01-24 14:20', user: 'Admin', action: 'DELETE', entity: 'Product', details: 'Ürün Silindi: YMH-OIL-001' },
                                        { id: 2, date: '2026-01-24 13:45', user: 'Kemal Usta', action: 'UPDATE', entity: 'Staff', details: 'Yetki Güncellendi: Serkan D.' },
                                    ].map(log => (
                                        <tr key={log.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                                            ...
                                            <td>
                                                <span style={{
                                                    ...
                                                    color: log.action === 'DELETE' ? 'var(--danger)' : 'var(--warning)'
                                                }}>
                                                    {log.action}
                                                </span>
                                            </td>
