...
export default function SettingsPage() {
    ...
    const [newCampaign, setNewCampaign] = useState<any>({
        ...
        discountRate: 0,
        pointsRate: 0,
        conditions: { brands: [], categories: [], paymentMethods: [] }
    });
    ...
    const addCampaign = async () => {
        ...
        try {
            ...
            if (res.ok) {
                showSuccess('Başarılı', 'Kampanya eklendi.');
                refreshCampaigns();
                setNewCampaign({ name: '', type: 'payment_method_discount', discountRate: 0, pointsRate: 0, conditions: { brands: [], categories: [], paymentMethods: [] } });
            }
                ...
                {activeTab === 'campaigns' && (
                    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
                        ...
                        <div className="card glass">
                            ...
                            <button onClick={addCampaign} className="btn btn-primary w-full" style={{ height: '48px', fontWeight: '900' }}>KAMPANYAYI YAYINLA</button>
                        </div>
                                ...
                                <tbody>
                                    {campaigns.map(camp => (
                                        <tr key={camp.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)' }}>
                                            ...
                                            <td style={{ textAlign: 'right' }}>
                                                <button onClick={() => deleteCampaign(camp.id)} className="btn btn-ghost" style={{ color: 'var(--danger)' }}>Sil</button>
                                            </td>
                                        </tr>
                                    ))}
