...
interface AppContextType {
    ...
    processSale: (saleData: { items: { productId: number | string, qty: number }[], total: number, kasaId: number | string, description: string, paymentMode?: string, customerId?: string | number, customerName?: string, installments?: number, pointsUsed?: number, couponCode?: string }) => Promise<boolean>;
    staff: Staff[];
        ...
        try {
            ...
            const res = await fetch('/api/sales/create', {
                ...
                body: JSON.stringify({
                    ...saleData,
                    items: enrichedItems,
                    total: saleData.total - totalDiscount,
                    customerName,
                    customerId: saleData.customerId || customer?.id,
                    paymentMode: saleData.paymentMode,
                    earnedPoints,
                    pointsUsed: (saleData as any).pointsUsed || 0,
                    couponCode: (saleData as any).couponCode
                })
            });
