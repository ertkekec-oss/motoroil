...
export default function SetupWizard() {
    ...
    const handleInstall = async () => {
        ...
        try {
            ...
            // 5. Create setup lock file

            // For now, save to localStorage (mock)
            localStorage.setItem('motoroil_setup_complete', 'true');
            localStorage.setItem('motoroil_config', JSON.stringify(setupData));

            setStep(5); // Success step
        } catch (error) {
            setErrors(['Kurulum sırasında bir hata oluştu. Lütfen tekrar deneyin.']);
