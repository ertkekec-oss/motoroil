...
export default function AccountingPage() {
    ...
    const { kasalar, setKasalar, addTransaction, currentUser, hasPermission, customers, suppliers, addFinancialTransaction, checks, addCheck, transactions, refreshCustomers } = useApp();
    ...
    const customerReceivables = useMemo(() => customers.filter(c => Number(c.balance) > 0).map(c => ({
        id: `cust-${c.id}`,
        title: `${c.name} (Alacak)`,
    ...
    const supplierPayables = useMemo(() => suppliers.filter(s => Number(s.balance) < 0).map(s => ({
        id: `sup-${s.id}`,
        title: `${s.name} (Borç)`,
    ...
    const saveNewCheck = async () => {
        ...
        try {
            await addCheck({
                ...
                customerId: newCheck.type.startsWith('Alınan') ? newCheck.entityId : undefined,
                supplierId: newCheck.type.startsWith('Verilen') ? newCheck.entityId : undefined,
    ...
    return (
        <div className="container" style={{ padding: '40px 20px' }}>
            <header className="flex-between mb-8">
                <div>
                    <h1 className="text-gradient">Muhasebe & Finans</h1>
                    <p className="text-muted">Nakit Akışı ve Hesap Yönetimi</p>
                </div>
                <div className="flex gap-4 items-center">
                    <button onClick={() => { refreshCustomers(); window.location.reload(); }} className="btn btn-outline text-xs">
                        🔄 Verileri Yenile
                    </button>
                    <div className="card" style={{ padding: '10px 20px', background: 'var(--bg-card)' }}>
                        <span className="text-muted">Net Kasa:</span>
                        <span style={{ fontWeight: 'bold', marginLeft: '8px', color: 'var(--success)' }}>
            ...
            <div className="flex gap-2 mb-6 border-b border-white/10">
                {['Alacaklar (Tahsilat)', 'Borçlar (Ödeme)', 'Çek & Senet', 'Banka & Kasa', 'Giderler'].map((label, i) => {
                    const keys = ['receivables', 'payables', 'checks', 'banks', 'expenses'];
                    const key = keys[i];
                    if (!isSystemAdmin && (key === 'payables' || key === 'checks')) return null;
                    const active = activeTab === key;
                    return (
                        <button key={key} onClick={() => setActiveTab(key)} style={{
                            padding: '12px 24px', background: active ? 'var(--bg-card)' : 'transparent',
            ...
            <div className="card glass p-6 min-h-[500px]">
                {activeTab === 'receivables' && (
                    <div>
                        ...
                        <table className="w-full text-left">
                            <thead className="text-muted text-xs border-b border-white/10"><tr><th className="p-3">Açıklama</th><th>Vade</th><th>Tutar</th><th>Durum</th><th></th></tr></thead>
                            <tbody>
                                {[...filterByBranch(receivables), ...customerReceivables].map(item => (
                                    <tr key={item.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                        ...
                                        <td className="text-right">
                                            ...
                                            {canDelete && item.id.toString().startsWith('manual-') && <button onClick={() => {
                                                if (isProcessing) return;
                                                setReceivables(receivables.filter(r => r.id !== item.id));
                                            }} disabled={isProcessing} className="btn-outline text-xs text-danger border-danger hover:bg-danger hover:text-white">Sil</button>}
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                ...
                {activeTab === 'payables' && (
                    <div>
                        ...
                        <table className="w-full text-left">
                            <thead className="text-muted text-xs border-b border-white/10"><tr><th className="p-3">Açıklama</th><th>Vade</th><th>Tutar</th><th>Durum</th><th></th></tr></thead>
                            <tbody>
                                {[...filterByBranch(payables), ...supplierPayables].map(item => (
                                    <tr key={item.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                        <td className="p-4"><div>{item.title}</div><div className="text-xs text-muted">{item.date}</div></td>
                                        ...
                                        <td className="text-right">
                                            ...
                                            {canDelete && item.id.toString().startsWith('manual-') && <button onClick={() => {
                                                if (isProcessing) return;
                                                setPayables(payables.filter(p => p.id !== item.id));
                                            }} disabled={isProcessing} className="btn-outline text-xs text-danger border-danger hover:bg-danger hover:text-white">Sil</button>}
                                        </td>
                                    </tr>
                                ))}
                            ...
                            <tbody>
                                {checks.map(c => (
                                    <tr key={c.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                        ...
                                        <td>{canDelete && <button disabled={isProcessing} onClick={() => {
                                            if (isProcessing) return;
                                            showConfirm(
                                                'Emin misiniz?',
                                        ...
                                        }} className="btn btn-outline text-xs text-danger border-danger hover:bg-danger hover:text-white">{isProcessing ? 'SİLİNİYOR...' : 'Sil'}</button>}</td>
                ...
                {activeTab === 'banks' && (
                    <div>
                        <div className="flex-between mb-6"><h3>Kasa & Banka</h3><button onClick={() => setShowVirmanModal(true)} className="btn btn-primary">🔄 Virman</button></div>
                        <div className="grid grid-cols-3 gap-4 mb-4">
                            {kasalar.map(k => (
                                <div key={k.id} className="card p-6 flex flex-col gap-4 transform hover:scale-105 transition-transform" style={{ background: k.type === 'Nakit' ? 'linear-gradient(135deg, #064e3b 0%, #10b981 100%)' : 'linear-gradient(135deg, #1e3a8a 0%, #3b82f6 100%)' }}>
                                    <div className="text-[10px] font-bold opacity-70 tracking-widest">{k.type.toUpperCase()}</div>
                                    <div className="flex-between">
                                        <b>{k.name}</b>
                                        <div className="flex gap-2 relative z-50">
                                            {canDelete && (
                                                <>
                                                    <button onClick={(e) => { e.stopPropagation(); handleEditKasa(k); }} className="text-xs btn-outline p-1 px-2 border-white/20 hover:bg-white/10 cursor-pointer" type="button">✏️</button>
                                                    <button onClick={(e) => { e.stopPropagation(); handleDeleteKasa(k.id.toString(), k.balance); }} className="text-xs btn-outline p-1 px-2 border-danger text-danger hover:bg-danger hover:text-white cursor-pointer" type="button">🗑️</button>
                                                </>
                                            )}
                                            <span style={{ fontSize: '24px' }}>{k.type === 'Nakit' ? '💵' : '🏦'}</span>
                                        </div>
                                    </div>
                                    <div className="text-2xl font-black">₺ {k.balance.toLocaleString()}</div>
                                </div>
                            ))}
                            {hasPermission('create_bank') && <div onClick={() => setShowAddBank(true)} className="card border-2 border-dashed border-white/10 flex-center cursor-pointer min-h-[140px] text-muted hover:border-primary hover:text-primary transition-colors">+ Yeni Kasa</div>}
                        </div>
                ...
                {activeTab === 'expenses' && (
                    <div>
                        <div className="flex-between mb-4"><h3>İşletme Giderleri</h3><button onClick={() => setShowExpenseModal(true)} className="btn btn-primary">+ Gider Ekle</button></div>
                        <table className="w-full text-left">
                            <thead className="text-muted text-xs border-b border-white/10"><tr><th className="p-3">Açıklama</th><th>Kategori</th><th>Tarih</th><th>Ödeme</th><th className="text-right">Tutar</th></tr></thead>
                            <tbody>
                                {expenses.map(e => (
                                    <tr key={e.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                        ...
                                        <td className="text-muted">{e.date}</td><td>{e.method}</td>
                                        <td className="text-right font-bold text-danger">
                                            {e.amount.toLocaleString()} ₺
                                            {canDelete && (
                                                <div className="inline-flex gap-2 ml-4">
                                                    <button onClick={() => handleEditExpense(e)} className="text-xs btn-outline p-1 px-2 border-white/20 hover:bg-white/10">✏️</button>
                                                    <button onClick={() => handleDeleteExpense(e.id)} className="text-xs btn-outline p-1 px-2 border-danger text-danger hover:bg-danger hover:text-white">🗑️</button>
                                                </div>
                                            )}
                                        </td>
                                    </tr>
            ...
            {showModal && (
                <div className="modal-overlay">
                    <div className="card glass animate-in modal-content">
                        <div className="flex-between mb-6"><h3>{modalType === 'receivable' ? '+ Tahsilat Ekle' : '- Ödeme Ekle'}</h3><button onClick={() => setShowModal(false)} className="close-btn">&times;</button></div>
                        <div className="flex flex-col gap-4">
                            <div className="form-group">
                                <label>CARİ SEÇİMİ</label>
                                <select value={newItem.entityId} onChange={e => setNewItem({ ...newItem, entityId: e.target.value })} className="input-field">
                                    <option value="">Lütfen Seçiniz...</option>
                                    {(modalType === 'receivable' ? customers : suppliers).map(c => <option key={c.id} value={c.id.toString()}>{c.name} (₺ {c.balance.toLocaleString()})</option>)}
                                </select>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div className="form-group"><label>TUTAR</label><input type="number" value={newItem.amount} onChange={e => setNewItem({ ...newItem, amount: e.target.value })} className="input-field font-bold text-lg" /></div>
                                <div className="form-group"><label>VADE / NOT</label><input type="text" value={newItem.due} onChange={e => setNewItem({ ...newItem, due: e.target.value })} className="input-field" /></div>
                            </div>
                            <div className="p-4 bg-white/5 rounded-xl border border-dashed border-white/10">
                                <div className="flex-between mb-4"><span className="text-sm font-bold">Direkt Finansal Kayıt Oluştur?</span><input type="checkbox" checked={newItem.isCollected} onChange={e => setNewItem({ ...newItem, isCollected: e.target.checked })} className="checkbox" style={{ width: '20px', height: '20px' }} /></div>
                                {newItem.isCollected && (
                                    <div className="flex flex-col gap-4">
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="form-group">
                                                <label>Nakit Tutar</label>
                                                <input type="number" placeholder="0.00" value={newItem.cashAmount} onChange={e => setNewItem({ ...newItem, cashAmount: e.target.value })} className="input-field" />
                                            </div>
                                            <div className="form-group">
                                                <label>Kasa Seçimi</label>
                                                <select value={newItem.selectedCashboxId} onChange={e => setNewItem({ ...newItem, selectedCashboxId: e.target.value })} className="input-field">
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="form-group">
                                                <label>Kredi Kartı Tutar</label>
                                                <input type="number" placeholder="0.00" value={newItem.cardAmount} onChange={e => setNewItem({ ...newItem, cardAmount: e.target.value })} className="input-field" />
                                            </div>
                                            <div className="form-group">
                                                <label>Banka Seçimi</label>
                                                <select value={newItem.selectedCardBankId} onChange={e => setNewItem({ ...newItem, selectedCardBankId: e.target.value })} className="input-field">
                            ...
                            <button onClick={saveNewItem} disabled={isProcessing} className="btn btn-primary w-full p-4 font-bold mt-2 h-14 text-lg">
            ...
            {showCheckModal && (
                <div className="modal-overlay">
                    <div className="card glass animate-in modal-content">
                        <div className="flex-between mb-6"><h3>🏷️ Yeni Çek / Senet</h3><button onClick={() => setShowCheckModal(false)} className="close-btn">&times;</button></div>
                        <div className="flex flex-col gap-4">
                            <div className="form-group">
                            ...
                            <div className="form-group">
                                ...
                                <input type="text" placeholder="Çek/Senet No" value={newCheck.number} onChange={e => setNewCheck({ ...newCheck, number: e.target.value })} className="input-field" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div className="form-group"><label>Tutar</label><input type="number" placeholder="0.00" value={newCheck.amount} onChange={e => setNewCheck({ ...newCheck, amount: e.target.value })} className="input-field" /></div>
                                <div className="form-group"><label>Vade Tarihi</label><input type="date" value={newCheck.dueDate} onChange={e => setNewCheck({ ...newCheck, dueDate: e.target.value })} className="input-field" /></div>
                            </div>
                            <div className="form-group">
                                <label>İlgili Cari</label>
                                <select value={newCheck.entityId} onChange={e => setNewCheck({ ...newCheck, entityId: e.target.value })} className="input-field">
                                    <option value="">Cari Seçin...</option>
                                    {(newCheck.type.startsWith('Alınan') ? customers : suppliers).map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                                </select>
                            </div>
                            <div className="form-group">
                                <label>Banka Adı (Opsiyonel)</label>
                                <input type="text" placeholder="Banka Adı" value={newCheck.bank} onChange={e => setNewCheck({ ...newCheck, bank: e.target.value })} className="input-field" />
                            </div>
            ...
            {showExpenseModal && (
                <div className="modal-overlay">
                    <div className="card glass animate-in modal-content">
                        <div className="flex-between mb-6"><h3>{isEditingExpense ? '✏️ Gider Düzenle' : '💸 Yeni Gider'}</h3><button onClick={() => setShowExpenseModal(false)} className="close-btn">&times;</button></div>
                        <div className="flex flex-col gap-4">
                            <div className="form-group">
                                <label>Gider Açıklaması</label>
                                <input type="text" placeholder="Örn: Kırtasiye malzemesi" value={newExpense.title} onChange={e => setNewExpense({ ...newExpense, title: e.target.value })} className="input-field" />
                            </div>
                            <div className="form-group">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="form-group">
                                    <label>Tutar</label>
                                    <input type="number" placeholder="0.00" value={newExpense.amount} onChange={e => setNewExpense({ ...newExpense, amount: e.target.value })} className="input-field" />
                                </div>
                                <div className="form-group">
                            ...
                            <button onClick={saveNewExpense} disabled={isProcessing} className="btn btn-primary w-full p-4 font-bold mt-2">
                                {isProcessing ? 'KAYDEDİLİYOR...' : 'KAYDET'}
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="card glass animate-in modal-content" style={{ width: '500px' }}>
                        ...
                        <div className="flex-col gap-4">
                            ...
                            <div className="form-group">
                                ...
                                <input type="text" className="input-field" placeholder="Örn: Gün sonu devir" value={virmanData.description} onChange={e => setVirmanData({ ...virmanData, description: e.target.value })} />
                            </div>
