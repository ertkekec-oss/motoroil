        ...
        <div className="container" style={{ padding: '40px 20px', maxWidth: '600px', margin: '0 auto' }}>
            <div className="card glass">
                ...
                <div className="flex-col gap-6">
                    ...
                    <div className="grid-cols-2" style={{ gap: '12px', gridTemplateColumns: '1fr 1fr' }}>
                        {/* Row 1 */}
                        <button
                            onClick={() => setPaymentMethod('cash')}
                            className={`card flex-col flex-center ${paymentMethod === 'cash' ? 'active-method' : ''}`}
                            style={{
                                padding: '16px', cursor: 'pointer', border: paymentMethod === 'cash' ? '2px solid var(--success)' : '1px solid var(--border-light)',
                                background: paymentMethod === 'cash' ? 'rgba(16, 185, 129, 0.1)' : 'transparent'
                            }}
                        >
                            <span style={{ fontSize: '24px' }}>💵</span>
                            <span style={{ fontSize: '12px', marginTop: '8px' }}>Nakit</span>
                        </button>
                        <button
                            onClick={() => setPaymentMethod('cc')}
                            className={`card flex-col flex-center ${paymentMethod === 'cc' ? 'active-method' : ''}`}
                            style={{
                                padding: '16px', cursor: 'pointer', border: paymentMethod === 'cc' ? '2px solid var(--primary)' : '1px solid var(--border-light)',
                                background: paymentMethod === 'cc' ? 'rgba(59, 130, 246, 0.1)' : 'transparent'
                            }}
                        >
                            <span style={{ fontSize: '24px' }}>💳</span>
                            <span style={{ fontSize: '12px', marginTop: '8px' }}>Kredi Kartı</span>
                        </button>

                        {/* Row 2 */}
                        <button
                            onClick={() => setPaymentMethod('iban')}
                            className={`card flex-col flex-center ${paymentMethod === 'iban' ? 'active-method' : ''}`}
                            style={{
                                padding: '16px', cursor: 'pointer', border: paymentMethod === 'iban' ? '2px solid #F59E0B' : '1px solid var(--border-light)',
                                background: paymentMethod === 'iban' ? 'rgba(245, 158, 11, 0.1)' : 'transparent'
                            }}
                        >
                            <span style={{ fontSize: '24px' }}>🏦</span>
                            <span style={{ fontSize: '12px', marginTop: '8px' }}>Havale / EFT</span>
                        </button>
                        <button
                            onClick={() => setPaymentMethod('account')}
                            className={`card flex-col flex-center ${paymentMethod === 'account' ? 'active-method' : ''}`}
                            style={{
                                padding: '16px', cursor: 'pointer', border: paymentMethod === 'account' ? '2px solid #EC4899' : '1px solid var(--border-light)',
                                background: paymentMethod === 'account' ? 'rgba(236, 72, 153, 0.1)' : 'transparent'
                            }}
                        >
                            <span style={{ fontSize: '24px' }}>📖</span>
                            <span style={{ fontSize: '12px', marginTop: '8px' }}>Cari Hesaba At (Veresiye)</span>
                        </button>
                    </div>
