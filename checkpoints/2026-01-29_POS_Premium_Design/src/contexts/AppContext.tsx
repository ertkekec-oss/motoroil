...
interface AppContextType {
    ...
    processSale: (saleData: { items: { productId: number | string, qty: number }[], total: number, kasaId: number | string, description: string, paymentMode?: string, customerId?: string | number }) => void;
    staff: Staff[];
...
export function AppProvider({ children }: { children: React.ReactNode }) {
    ...
    const processSale = async (saleData: { items: { productId: number | string, qty: number }[], total: number, kasaId: number | string, description: string, paymentMode?: string, customerId?: string | number }) => {
        ...
        try {
            ...
            const customerName = saleData.description.includes('POS Satışı:') ?
                saleData.description.split(':').pop()?.split('(')[0].trim() : 'Perakende';
