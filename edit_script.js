...
content = content.replace(
    ...
    "{ group: 'SERVİS & VARLIKLAR', items: [{ id: 'services', label: 'Servis & Cihazlar' }, { id: 'documents', label: 'Dosyalar' }] }"
);

// Delete the warranties activeTab check logic entirely
const warrantiesTabMatch = ") : activeTab === 'warranties' ? (";
const checksTabMatch = ") : activeTab === 'checks' ? (";
const indexStart = content.indexOf(warrantiesTabMatch);
const indexEnd = content.indexOf(checksTabMatch);

if (indexStart !== -1 && indexEnd !== -1) {
    // Extract the warranties tab code
    const deletedContent = content.substring(indexStart, indexEnd);
    content = content.substring(0, indexStart) + content.substring(indexEnd);
}

// Now replace the activeTab === 'services' start to include the Varlıklar layout
const servicesTabStartMatch = `                    {activeTab === 'services' ? (
                        <div className="p-4 sm:p-6">
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                                <h3 style={{ margin: 0, color: 'var(--text-main, #fff)', fontSize: '20px', fontWeight: '800' }}>Servis Kayıtları</h3>`;

const newServicesTabStart = `                    {activeTab === 'services' ? (
                        <div className="p-4 sm:p-6">
...
content = content.replace(servicesTabStartMatch, newServicesTabStart);
