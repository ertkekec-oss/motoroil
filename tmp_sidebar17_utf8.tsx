...
export default function Sidebar() {
    ...
    const isSystemAdmin = currentUser === null || currentUser?.role === 'SUPER_ADMIN' || (currentUser?.role && (currentUser.role.toLowerCase().includes('admin') || currentUser.role.toLowerCase().includes('m├╝d├╝r')));
    ...
    const permMap: Record<string, { perm?: string, feature?: string }> = {
        ...
        '/staff/me': { perm: 'pos_access' }, // Herkes eri┼şebilir genelde POS yetkisi olanlar
    ...
    const menuItems = [
        { name: 'POS Terminal', href: '/', icon: '­şÅ«' },
        { name: 'Personel Paneli', href: '/staff/me', icon: '­şæñ' },

        // FINANSAL Y├ûNET─░M GRUBU
        { name: 'Finansal Y├Ânetim', href: '/accounting', icon: '­şÅø´©Å' },
        { name: 'Sat─▒┼ş Y├Ânetimi', href: '/sales', icon: '­şğ¥' },
        { name: 'Cari Hesaplar', href: '/customers', icon: '­şñØ' },
        { name: 'Tedarik├ği A─ş─▒', href: '/suppliers', icon: '­şÜÜ' },

        // AKILLI S─░STEMLER GRUBU
        { name: 'Finansal Kontrol Kulesi', href: '/fintech/control-tower', icon: '­şù╝' },

        // OPERASYON GRUBU
        { name: 'Envanter & Depo', href: '/inventory', icon: '­şôÑ' },
        {
            name: 'Saha Sat─▒┼ş Y├Ânetimi',
            icon: '­şù║´©Å',
            isParent: true,
            id: 'field-sales-parent',
            subItems: [
                { name: 'Y├Ânetim Paneli', href: '/field-sales/admin/routes', icon: 'ÔÜÖ´©Å' },
                { name: 'Saha Sat─▒┼ş Paneli', href: '/field-sales', icon: '­şôı' },
                { name: 'Canl─▒ Saha Takibi', href: '/field-sales/admin/live', icon: '­şø░´©Å' },
            ]
        },
        { name: 'Teklifler', href: '/quotes', icon: '­şôï' },
        { name: 'Servis Masas─▒', href: '/service', icon: '­şøá´©Å' },
        ...
        // ANAL─░Z & DENET─░M
        {
            ...
            id: 'reports-parent',
            subItems: [
                { name: '─░┼ş Zekas─▒ (CEO)', href: '/reports/ceo', icon: '­şğá' },
                { name: 'Veri Analizi', href: '/reports', icon: '­şôè' },
            ]
        },
        { name: 'PDKS Y├Ânetimi', href: '/staff/pdks', icon: '­şøí´©Å' },
        { name: 'Denetim Kay─▒tlar─▒', href: '/admin/audit-logs', icon: '­şöı' },
        { name: 'Ka├ğak Sat─▒┼ş Tespit', href: '/security/suspicious', icon: '­şÜ¿' },

        // S─░STEM & AYARLAR
        { name: 'Mali M├╝┼şavir', href: '/advisor', icon: '­şÆ╝' },
        { name: 'Sistem Ayarlar─▒', href: '/settings', icon: 'ÔÜÖ´©Å' },
        { name: 'Ekip & Yetki', href: '/staff', icon: '­şæÑ' },
        { name: 'Abonelik & Planlar', href: '/billing', icon: '­şÆÄ' },
        { name: 'Yard─▒m & K─▒lavuz', href: '/help', icon: 'ÔØô' },
        ...(isPlatformAdmin ? [
            { name: 'Destek Masas─▒ (Inbox)', href: '/admin/support/tickets', icon: '­şôÑ' },
            { name: 'Bilgi Bankas─▒ Y├Ânetimi', href: '/admin/tenants/PLATFORM_ADMIN/help', icon: '­şôÜ' }
        ] : []),
    ].filter(item => {
                    ...
                    {isPlatformAdmin && (
                        <div>
                            ...
                            <select
                                ...
                                <option value="null">­şÅó T├£M S─░STEM</option>
                                {availableTenants.map(t => (
                                    <option key={t.id} value={t.id}>
                                        ­şæÑ {t.name?.toUpperCase()}
                                    </option>
                                ))}
                            </select>
                        </div>
                            ...
                            {branches.length > 0 ? (
                                branches.map(b => (
                                    <option key={b.id} value={b.name}>
                                        {b.type === 'Merkez' ? '­şÅó' : '­şöğ'} {b.name.toUpperCase()}
                                    </option>
                                ))
                            ) : (
                                <option value="">{activeTenantId ? '┼ŞUBE Y├£KLEN─░YOR...' : '├ûNCE ┼Ş─░RKET SE├ç─░N'}</option>
                            )}
                        </select>
                    </div>
                </div>
                        ...
                        return (
                            <div key={item.id} style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                ...
                                >
                                    ...
                                    <span style={{ fontSize: '10px', transform: isExpanded ? 'rotate(180deg)' : 'none', transition: '0.3s' }}>Ôû╝</span>
                                </div>
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '20px' }}>
                    <div style={{
                        ...
                        {(displayUser.role?.includes('Admin') || currentUser === null) ? 'ÔÜí' : '­şæñ'}
                    </div>
                    <div style={{ flex: 1 }}>
