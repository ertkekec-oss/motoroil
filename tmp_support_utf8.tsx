...
const STATUS_LABELS: Record<string, string> = {
    NEW: 'Yeni',
    IN_PROGRESS: '─░┼şlemde',
    WAITING_CUSTOMER: 'Yan─▒t─▒n─▒z─▒ Bekliyor',
    RESOLVED: '├ç├Âz├╝mlendi',
    CLOSED: 'Kapat─▒ld─▒',
};

const PRIORITY_LABELS: Record<string, string> = {
    P1_URGENT: 'ÔÇ╝´©Å Acil',
    P2_HIGH: 'ÔØù´©Å Y├╝ksek',
    P3_NORMAL: 'Normal',
    P4_LOW: 'D├╝┼ş├╝k',
};
        ...
        <div className="p-6 md:p-8 max-w-6xl mx-auto font-sans">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                <div>
                    ...
                    <p className="text-gray-400 text-sm mt-1">Sormu┼ş oldu─şunuz t├╝m destek taleplerinin durumu.</p>
                </div>
            ...
            <div className="bg-[#0f111a] border border-white/5 rounded-2xl shadow-xl overflow-hidden">
                {tickets.length === 0 ? (
                    <div className="p-12 text-center">
                        <span className="text-4xl mb-4 block">­şÄğ</span>
                        <h3 className="text-white font-bold text-lg">Hen├╝z bir talebiniz yok</h3>
                        <p className="text-sm text-gray-500 mt-2">Yard─▒ma ihityac─▒n─▒z oldu─şunda bize her zaman ula┼şabilirsiniz.</p>
                    </div>
                ) : (
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-white/5 bg-white/5 text-gray-400 text-xs font-bold uppercase tracking-wider">
                                    ...
                                    <th className="px-6 py-4">├ûncelik</th>
                                    <th className="px-6 py-4">Son G├╝ncelleme</th>
                                </tr>
                            </thead>
                            <tbody>
                                {tickets.map(ticket => (
                                            ...
                                            <Link href={`/support/${ticket.id}`} className="block">
                                                ...
                                                {ticket.relatedHelpTopic && (
                                                    <div className="text-xs text-gray-500 mt-1 flex items-center gap-1">
                                                        <span>­şöù</span> ─░lgili d├Âk├╝man: {ticket.relatedHelpTopic.title}
                                                    </div>
                                                )}
                                            </Link>
                                        </td>
