                ...
                try {
                    ...
                    if (!response.ok) {
                        ...
                        if (response.status === 401) {
                            throw new Error(`HB_AUTH_ERROR: Hepsiburada Yetkilendirme Hatas─▒ (401). L├╝tfen API User ve Secret Key bilgilerini kontrol edin.`);
                        }
                        if (response.status === 429) {
                            throw new Error(`HB_RATE_LIMIT: Hepsiburada API limitine tak─▒ld─▒n─▒z. (429)`);
                        }

                        throw new Error(`HB_API_ERROR: ${target.name} servisi ${response.status} hatas─▒ d├Ând├╝rd├╝: ${errText.substring(0, 200)}`);
                    }
    ...
    private mapOrder(hbOrder: any): MarketplaceOrder {
        try {
            return {
                ...
                customerName: hbOrder.customer?.name || 'M├╝┼şteri',
