...
async function main() {
    ...
    const paths = [
        `/einvoice/Purchase/Details?uuid=${uuid}`,
        `/einvoice/Purchase/Model?uuid=${uuid}`,
        `/einvoice/Purchase/Incoming?uuid=${uuid}`,
        `/einvoice/Purchase?uuid=${uuid}`
    ];
    ...
    fs.writeFileSync('nilvera_results_6.txt', output);
}
main();
