...
async function main() {
    ...
    console.log('Fixed tenantId for platform admin topics.');
...
> *Tediye (Ödeme) işlemleri de tedarikçileriniz için aynı ekran üzerinden **Ödeme Ekle** butonuyla yapılır.*`
    ...
    for (const catData of initialContent) {
        let cat = await prisma.helpCategory.findUnique({
        ...
        for (const topicData of catData.topics) {
            const exist = await prisma.helpTopic.findUnique({
            ...
            if (!exist) {
                await prisma.helpTopic.create({
                    data: {
                        ...
                        body: topicData.body,
                        status: topicData.status as any,
                        order: topicData.order,
                        tenantId: null // explicitly global!
                    }
                });
                console.log('Created topic:', topicData.title);
            } else {
                // update if exist to ensure tenantId is null
