...
// The file is fairly structured. I'll read and reconstruct the entire JSX tree inside the `return (` block.
...
const tabAllRegex = /<table style=\{\{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '800px' \}\}>\s*<thead>\s*<tr[\s\S]*?>\s*<th[\s\S]*?>TARİH & TÜR<\/th>\s*<th[\s\S]*?>AÇIKLAMA \/ REFERANS<\/th>\s*<th[\s\S]*?>TUTAR<\/th>\s*<th[\s\S]*?>İŞLEM<\/th>\s*<\/tr>\s*<\/thead>\s*<tbody>([\s\S]*?)<\/tbody>\s*<\/table>/;
const matchAll = data.match(tabAllRegex);
if(matchAll) {
    let tbodyContent = matchAll[1];

    // Replace the <tr> empty state check `{paginatedList.length === 0 && (` with empty state direct mapping inside JSX
    tbodyContent = tbodyContent.replace(/\{paginatedList\.length === 0 && \([\s\S]*?<\/tr>\s*\)\}/, '');
    
    // Replace inline tr styles
    tbodyContent = tbodyContent.replace(/<tr key=\{idx\} style=\{\{ borderBottom: '1px solid var\(--border-color, rgba\(255,255,255,0\.03\)\)', transition: 'background 0\.2s' \}\} className="hover:bg-white\/5">/g, 
    ...
    // Replace inline td styles
    tbodyContent = tbodyContent.replace(/<td style=\{\{ padding: '20px' \}\}>/g, `<td className="px-5 py-3 align-middle text-[12px] font-semibold text-slate-600 dark:text-slate-400">`);
    tbodyContent = tbodyContent.replace(/<td style=\{\{ padding: '20px', fontSize: '14px', color: 'var\(--text-main, #eee\)' \}\}>/g, `<td className="px-5 py-3 align-middle text-[12px] font-semibold text-slate-600 dark:text-slate-400">`);
    tbodyContent = tbodyContent.replace(/<td style=\{\{ padding: '20px', textAlign: 'right', fontWeight: '800', color: \(item\.amount < 0 && item\.type !== 'Ödeme'\) \? '#ef4444' : '#10b981', fontSize: '16px' \}\}>/g, `<td className="px-5 py-3 align-middle text-right text-[14px] font-extrabold" style={{ color: (item.amount < 0 && item.type !== 'Ödeme') ? '#ef4444' : '#10b981' }}>`);
    tbodyContent = tbodyContent.replace(/<td style=\{\{ padding: '20px', textAlign: 'center' \}\}>/g, `<td className="px-5 py-3 align-middle">`);

    // Replace action buttons
    tbodyContent = tbodyContent.replace(/style=\{\{ fontSize: '11px', padding: '6px 12px', background: 'var\(--bg-card, rgba\(255,255,255,0\.05\)\)', border: '1px solid var\(--border-color, rgba\(255,255,255,0\.1\)\)', color: 'var\(--text-main, #fff\)', borderRadius: '8px', cursor: 'pointer', fontWeight: '700' \}\}/g, 
        `className="px-3 py-1.5 bg-slate-50 border border-slate-200 hover:bg-slate-100 dark:bg-white/5 dark:border-white/10 dark:hover:bg-white/10 text-slate-700 dark:text-white rounded-[6px] text-[11px] font-bold transition-colors"`
    );
    tbodyContent = tbodyContent.replace(/style=\{\{\s*fontSize: '11px',\s*padding: '6px 12px',\s*background: 'rgba\(59, 130, 246, 0\.1\)',\s*border: '1px solid rgba\(59, 130, 246, 0\.3\)',\s*color: '#3b82f6',\s*borderRadius: '8px',\s*cursor: 'pointer',\s*fontWeight: '700'\s*\}\}/g, 
...
<div className="overflow-auto max-h-[calc(100vh-270px)] custom-scroll bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-white/5 rounded-[20px] shadow-sm flex flex-col mb-4">
    <table className="w-full text-left border-collapse">
        ...
        <tbody className="divide-y divide-slate-100 dark:divide-white/5">
            {paginatedList.length === 0 ? (
                <tr><td colSpan={4} className="px-6 py-10 text-center text-slate-500 dark:text-slate-400 font-semibold text-[14px]">Henüz bir işlem kaydı bulunmuyor.</td></tr>
            ) : (
                ${tbodyContent}
            )}
        </tbody>
    ...
    data = data.replace(tabAllRegex, newTabAll);
}

// 5. Tab 2: Checks Table Rewrite
const tabChecksRegex = /<table style=\{\{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '800px' \}\}>\s*<thead>\s*<tr[\s\S]*?>\s*<th[\s\S]*?>VADE VE BANKA<\/th>\s*<th[\s\S]*?>DURUM & TÜR<\/th>\s*<th[\s\S]*?>TUTAR<\/th>\s*<th[\s\S]*?>İŞLEM<\/th>\s*<\/tr>\s*<\/thead>\s*<tbody>([\s\S]*?)<\/tbody>\s*<\/table>/;
const matchChecks = data.match(tabChecksRegex);
if(matchChecks) {
    let tbodyContent2 = matchChecks[1];
    
    // Replace the ternary emptiness
    tbodyContent2 = tbodyContent2.replace(/\{paginatedList\.length === 0 \? \([\s\S]*?<\/tr>\s*\)\s*:\s*\(\s*paginatedList\.map\(\(c: any\) => \(/, '');
    tbodyContent2 = tbodyContent2.replace(/\)\s*\)\s*\}/, '');

    tbodyContent2 = tbodyContent2.replace(/<tr key=\{c\.id\} style=\{\{ borderBottom: '1px solid var\(--border-color, rgba\(255,255,255,0\.03\)\)', transition: 'background 0\.2s' \}\} className="hover:bg-white\/5">/g, 
    ...
    tbodyContent2 = tbodyContent2.replace(/<td style=\{\{ padding: '20px' \}\}>/g, `<td className="px-5 py-3 align-middle text-[12px] font-semibold text-slate-600 dark:text-slate-400">`);
    tbodyContent2 = tbodyContent2.replace(/<td style=\{\{ padding: '20px', textAlign: 'right', fontWeight: '800', color: '#3b82f6', fontSize: '16px' \}\}>/g, `<td className="px-5 py-3 align-middle text-right text-[14px] font-extrabold text-blue-600 dark:text-blue-400">`);
    tbodyContent2 = tbodyContent2.replace(/<td style=\{\{ padding: '20px', textAlign: 'center' \}\}>/g, `<td className="px-5 py-3 align-middle">`);

    // Button style
    tbodyContent2 = tbodyContent2.replace(/style=\{\{ fontSize: '11px', padding: '8px 16px', background: '#3b82f6', border: 'none', color: 'white', borderRadius: '8px', cursor: 'pointer', fontWeight: '800', boxShadow: '0 4px 10px rgba\(59, 130, 246, 0\.2\)' \}\}/g, 
...
<div className="overflow-auto max-h-[calc(100vh-270px)] custom-scroll bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-white/5 rounded-[20px] shadow-sm flex flex-col mb-4">
    ...
    data = data.replace(tabChecksRegex, newTabChecks);
}

// Remove old pagination wrappers since we have mb-4 inside table wrapper, we can put pagination below it cleanly.
data = data.replace(/<div style=\{\{ padding: '20px', borderTop: '1px solid var\(--border-color, rgba\(255,255,255,0\.05\)\)' \}\}>\s*<Pagination currentPage=\{currentPage\} totalPages=\{totalPages\} onPageChange=\{setCurrentPage\} \/>\s*<\/div>/g, 
    `<div className="flex justify-center mt-4">
        <Pagination currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} />
    </div>`
);

// We should also replace `<div style={{ overflowX: 'auto' }}>` that wrapped the tables since we moved the scroll overflow into the table wrapper itself.
data = data.replace(/<div style=\{\{ overflowX: 'auto' \}\}>([\s\S]*?)<\/div>/, '$1');

// Change `background: 'var(--bg-main)', color: 'var(--text-main)'`
...
console.log('Supplier detail fixed');
