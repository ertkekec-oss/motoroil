...
const perfPagePath = path.join(process.cwd(), 'src', 'app', '(app)', 'staff', 'performance', 'page.tsx');
...
content = content.replace(checkRegex, `if ((!data || !data.assignments || data.assignments.length === 0) && (!standardTargets || standardTargets.length === 0)) {`);
