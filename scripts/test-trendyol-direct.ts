...
async function main() {
    ...
    let url = `${srv['baseUrl']}/integration/finance/che/sellers/${srv['config'].supplierId}/settlements?startDate=${startDate}&endDate=${endDate}&orderNumber=11069830379`;
    console.log(url);
    let response = await fetch(url, { headers: srv['getHeaders']() });
    console.log("TEST 3 - 14 Days:\nSTATUS:", response.status, "\nBODY:", await response.text());
