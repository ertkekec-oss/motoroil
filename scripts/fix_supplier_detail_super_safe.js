...
data = data.replace(/\{item\.date\}<\/div>\s*<span/g, `{item.date}</div>\n<span`); // Should be already handled below
// Actually the previous missing `</div>` was MY FAULT string replace. The original file has:
// `<div style={{ fontWeight: '700', color: 'var(--text-main, #e2e8f0)', marginBottom: '4px' }}>{item.date}</div>`
// My regex matched correctly. It was only the `)}` that destroyed the end.

data = data.replace(/\{paginatedList\.length === 0 && \(\s*<tr><td colSpan=\{4\} style=\{\{ textAlign: 'center', padding: '60px', color: 'var\(--text-muted, #888\)' \}\}>Henüz bir işlem kaydı bulunmuyor\.<\/td><\/tr>\s*\)\}/g, 
...
// Original: `<div style={{ padding: '20px', borderTop: '1px solid var(--border-color, rgba(255,255,255,0.05))' }}>`
...
// Replace action buttons within tables
...
// We need to resolve `{item.date}` syntax. Let's see if our new replace missed `.date`. Our replaces target `padding: '20px'`, which encompasses the cell, but leaves the inner divs intact.

fs.writeFileSync(file, data);
console.log('Supplier detail fixed safely (2)');
