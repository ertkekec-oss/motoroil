...
async function main() {
    ...
    const paths = [
        `/einvoice/Model/${uuid}`,
        `/einvoice/Model/Details/${uuid}`,
        `/general/EInvoice/Model/${uuid}`,
        `/general/Model/${uuid}`
    ];
    ...
    for (const p of paths) {
        try {
            ...
            if (r.data.InvoiceLines || r.data.EInvoice?.InvoiceLines) output += `   -> HAS LINES!\n`;
        } catch (e: any) {
            output += `Path: ${p} | Error: ${e.response?.status}\n`;
        }
    }
    fs.writeFileSync('nilvera_results_4.txt', output);
    console.log("Done");
}
main();
