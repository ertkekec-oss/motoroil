...
const mePagePath = path.join(__dirname, 'src', 'app', '(app)', 'staff', 'me', 'page.tsx');
let content = fs.readFileSync(mePagePath, 'utf8');

// 1. Add targets fetching logic to the main StaffPortal function
// Let's find: const [pdksStatus, setPdksStatus] = useState<any>(null);
...
content = content.replace(kpiAreaRegex, `<EnterpriseCard className="p-6 border-l-4" borderLeftColor="#3b82f6">
            ...
            <EnterpriseCard className="p-6 border-l-4" borderLeftColor="#ef4444">
                ...
                <p className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">
                    {targets.filter((t: any) => t.type === 'VISIT').reduce((acc: number, t: any) => acc + t.currentValue, 0)} <span className="text-lg text-slate-400 font-bold">Adet</span>
                </p>
...
content = content.replace(targetBarsRegex, `
                        {targets.length === 0 ? (
...
// 5. Inject tags to <DashboardView />
