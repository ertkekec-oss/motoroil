const fs = require('fs');
const filePath = 'src/app/(app)/test-desktop/ClientDashboard.tsx';

const content = fs.readFileSync(filePath, 'utf-8');
const lines = content.split(/\r?\n/);

// Keep lines 0 to 516 (inclusive is up to 516 => index 516 is line 517). Wait. 
// My previous comment was index 516 is line 517.
// Let's find exactly the line.
let startIndex = lines.findIndex(l => l.includes('{/* WIDGET GRID */}'));
let endIndex = lines.findIndex((l, idx) => idx > startIndex && l.includes('{/* 8. BİLDİRİMLER VE ONAYLAR (GENİŞLETİLMİŞ) */}'));
if (endIndex === -1) {
    // try to find where the grid ends just before </div></div></div></div>
    endIndex = 781;
}

const newLines = lines.slice(0, startIndex);

const widgetGridCode = `                    {/* WIDGET GRID */}
...
const widgetGridLines = widgetGridCode.split('\\n');
const endLines = lines.slice(endIndex);

const finalLines = [...newLines, ...widgetGridLines, ...endLines];

fs.writeFileSync(filePath, finalLines.join('\\n'), 'utf-8');
console.log('Update complete. Start: ' + startIndex + ' End: ' + endIndex);
