...
// The marker for MODERN_TABS inner fields is "Your Pie Chart"
const tabsMarker = `updateBlockData('chartFilter', e.target.value)} className="w-full bg-slate-900 border border-slate-700/50 p-1.5 text-[10px] text-white rounded outline-none h-7" />
                           </div>
                        </div>`;

const tabsReplacement = `updateBlockData('chartFilter', e.target.value)} className="w-full bg-slate-900 border border-slate-700/50 p-1.5 text-[10px] text-white rounded outline-none h-7" />
                           </div>
...
if(editorContent.includes(tabsMarker)) {
    editorContent = editorContent.replace(tabsMarker, tabsReplacement);
    console.log("TABS SUCCESS");
} else {
    console.log("TABS FAILED");
    // fallback test
    if (editorContent.includes("balanceFlagUrl")) console.log("TABS ALREADY THERE");
}

// The marker for MODERN_INTEGRATIONS is "Yorumlar Açıklama"
const integMarker = `updateBlockData('testimonialDesc', e.target.value)} className="w-full bg-slate-900 border border-slate-700/50 rounded p-2 text-xs text-white" />
                        </div>`;

const integReplacement = `updateBlockData('testimonialDesc', e.target.value)} className="w-full bg-slate-900 border border-slate-700/50 rounded p-2 text-xs text-white" />
...
if(editorContent.includes(integMarker)) {
    editorContent = editorContent.replace(integMarker, integReplacement);
    console.log("INTEG SUCCESS");
} else {
    console.log("INTEG FAILED");
    if(editorContent.includes("Dikey Resim")) {
        console.log("INTEG ALREADY THERE");
    }
}
