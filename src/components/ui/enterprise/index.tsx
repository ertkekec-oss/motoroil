...
export const EnterpriseCard = ({ children, className = '', noPadding = false, borderLeftColor = '' }: any) => {
    return (
        <div
            className={`bg-white dark:bg-[#111827] rounded-[20px] border border-slate-200 dark:border-slate-800 shadow-sm ${noPadding ? '' : 'p-8'} ${className}`}
...
export const EnterpriseInput = React.forwardRef(({ className = '', label, ...props }: any, ref: any) => {
    return (
        <div className={`space-y-2 ${className}`}>
            ...
            <input
                ref={ref}
                className="w-full h-[44px] px-[12px] bg-white dark:bg-[#0B1120] border border-slate-200 dark:border-slate-700 rounded-[12px] text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-600 focus:border-blue-600 hover:border-slate-300 dark:hover:border-slate-600 transition-all outline-none disabled:opacity-50"
                {...props}
            />
        </div>
    );
...
export const EnterpriseSelect = React.forwardRef(({ className = '', label, children, ...props }: any, ref: any) => {
    return (
        <div className={`space-y-2 ${className}`}>
            ...
            <select
                ref={ref}
                className="w-full h-[44px] px-[12px] bg-white dark:bg-[#0B1120] border border-slate-200 dark:border-slate-700 rounded-[12px] text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-600 focus:border-blue-600 hover:border-slate-300 dark:hover:border-slate-600 transition-all outline-none appearance-none cursor-pointer disabled:opacity-50"
                {...props}
            >
                {children}
            </select>
...
export const EnterpriseTextarea = React.forwardRef(({ className = '', label, ...props }: any, ref: any) => {
    return (
        <div className={`space-y-2 ${className}`}>
            ...
            <textarea
                ref={ref}
                className="w-full p-[12px] bg-white dark:bg-[#0B1120] border border-slate-200 dark:border-slate-700 rounded-[12px] text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-600 focus:border-blue-600 hover:border-slate-300 dark:hover:border-slate-600 transition-all outline-none resize-y disabled:opacity-50"
                {...props}
            />
        </div>
    );
...
export const EnterpriseButton = ({ children, variant = 'primary', className = '', ...props }: any) => {
    let baseStyles = "px-6 h-[44px] rounded-[14px] font-bold text-sm transition-all flex items-center justify-center gap-2 active:scale-95 disabled:opacity-50 cursor-pointer outline-none shadow-sm";
    ...
    let variants: any = {
        ...
        danger: "bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 text-red-600 hover:bg-red-100 dark:hover:bg-red-500/30",
        warning: "bg-amber-50 rounded-xl text-amber-600 border border-amber-200 dark:bg-amber-500/10 dark:text-amber-500 dark:border-amber-500/20 hover:bg-amber-100 dark:hover:bg-amber-500/30",
...
export const EnterpriseSwitch = ({ checked, onChange, label, description, className = '' }: any) => {
    return (
        <label className={`flex items-center gap-5 p-5 bg-slate-50 dark:bg-white/[0.02] hover:bg-slate-100 dark:hover:bg-white/[0.05] rounded-[16px] border border-slate-200 dark:border-white/5 cursor-pointer transition-all select-none group ${className}`}>
...
export const EnterpriseTable = ({ headers, children, className = '' }: any) => {
    return (
        <div className={`w-full overflow-x-auto rounded-[16px] border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#111827] shadow-sm ${className}`}>
...
export const EnterpriseModal = ({ isOpen, onClose, title, children, footer, className = '' }: any) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 dark:bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className={`bg-white dark:bg-[#111827] rounded-[24px] border border-slate-200 dark:border-slate-800 shadow-2xl w-full max-w-lg overflow-hidden flex flex-col animate-in zoom-in-95 duration-200 ${className}`}>
                <div className="flex-between p-6 border-b border-slate-100 dark:border-slate-800/80">
