...
export const EnterpriseCard = ({
    ...
    return (
        <div
            className={`bg-white dark:bg-[#0F172A] border border-slate-200/80 dark:border-slate-800 rounded-2xl shadow-sm ${noPadding ? '' : 'p-6'} ${className}`}
...
export const EnterpriseSectionHeader = ({
    ...
    return (
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 border-b border-slate-200 dark:border-slate-800 pb-6 mb-6">
            {icon && (
                <div className="w-12 h-12 rounded-xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-2xl border border-slate-200 dark:border-slate-700 shrink-0">
                    {icon}
                </div>
            )}
...
export const EnterpriseInput = React.forwardRef<HTMLInputElement, React.InputHTMLAttributes<HTMLInputElement> & { label?: string; hint?: string; error?: string }>(
    ({ className = '', label, hint, error, ...props }, ref) => {
        const inputEl = (
            <input
                ref={ref}
                className={`w-full h-10 px-3.5 bg-white dark:bg-[#0B1220] border ${error ? 'border-rose-400 focus:ring-rose-400/30' : 'border-slate-200 dark:border-slate-800 focus:ring-slate-400/30'} rounded-xl text-sm text-slate-900 dark:text-slate-100 focus:ring-2 focus:border-slate-400 dark:focus:border-slate-600 transition-all outline-none disabled:opacity-50 placeholder:text-slate-400 dark:placeholder:text-slate-600 ${className}`}
                {...props}
            />
        );
        if (label || hint || error) {
...
export const EnterpriseButton = ({
    ...
    const baseStyles = "h-10 px-4 rounded-xl font-semibold text-sm transition-all flex items-center justify-center gap-2 active:scale-[0.98] disabled:opacity-50 cursor-pointer outline-none";

    const variants: Record<string, string> = {
        primary: "bg-slate-900 hover:bg-slate-800 text-white dark:bg-white dark:hover:bg-slate-100 dark:text-slate-900",
        secondary: "bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800",
...
export const EnterpriseSwitch = ({
    ...
    return (
        <label className={`flex items-center gap-3 p-4 bg-slate-50 dark:bg-slate-800/40 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-800 cursor-pointer transition-all select-none ${className}`}>
            <div className={`w-10 h-5.5 rounded-full relative transition-all duration-200 shrink-0 ${checked ? 'bg-slate-900 dark:bg-white' : 'bg-slate-200 dark:bg-slate-700'}`}
                style={{ height: '22px' }}>
                <div className={`absolute top-[3px] w-4 h-4 rounded-full transition-all duration-200 shadow-sm ${checked ? 'left-[22px] bg-white dark:bg-slate-900' : 'left-[3px] bg-white dark:bg-slate-400'}`} />
            </div>
...
export const EnterpriseTable = ({
    ...
    return (
        <div className={`w-full overflow-x-auto rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0F172A] ${className}`}>
...
// ─── EnterprisePageShell ───────────────────────────────────────────────────────
// Tüm Settings sayfaları için root container
...
// ─── EnterpriseTabs ────────────────────────────────────────────────────────────
// Enterprise standardı tab bar + tab butonları
export const EnterpriseTabs = ({
    ...
    return (
        <div className={`flex flex-wrap gap-1 p-1 bg-white dark:bg-[#0F172A] rounded-2xl border border-slate-200 dark:border-slate-800 w-fit mb-6 ${className}`}>
            {tabs.map((tab) => (
                <button
                    key={tab.id}
...
export const EnterpriseEmptyState = ({
    ...
    return (
        <div className={`flex flex-col items-center justify-center py-16 px-6 text-center ${className}`}>
            {icon && (
                <div className="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-2xl border border-slate-200 dark:border-slate-700 mb-4">
                    {icon}
                </div>
            )}
