...
export default function HubCatalogTabs() {
  ...
  return (
    <div className="flex items-center gap-1 border-b border-slate-200  mb-8 pb-px">
      {tabs.map((tab) => {
        const isActive =
          pathname === tab.href ||
          (tab.href !== "/catalog"
        ...
        return (
          ...
          >
            ...
            {tab.name}
            {isActive && (
              <div className="absolute bottom-0 left-0 w-full h-[3px] bg-indigo-600  rounded-t-full shadow-[0_-2px_8px_rgba(79,70,229,0.4)]"></div>
            )}
          </Link>
        );
      })}
