  ...
  if (proposals.length === 0) {
    return (
      <div className="p-6 bg-white rounded-xl border border-gray-100 flex items-center justify-center text-gray-500">
        <span>
          No trade proposals generated yet. Waiting for liquidity matches...
        </span>
      </div>
    );
  }
            ...
            <div className="flex justify-between items-start mb-2">
              <div className="flex flex-col">
                ...
                <span className="text-sm text-gray-500 mt-0.5">
                  Proposed Vol: {prop.proposedQuantityLow} -{" "}
                  {prop.proposedQuantityHigh} Units
                </span>
              </div>
              <div
                className={`px-2 py-1 rounded text-xs font-medium border ${prop.status === "ACCEPTED" ? "bg-green-50 text-green-700 border-green-200" : "bg-gray-50 text-gray-700 border-gray-200"}`}
              >
            ...
            <div className="grid grid-cols-2 gap-4 my-3 text-sm">
              <div className="bg-gray-50 p-2 rounded border border-gray-100">
                ...
                <span className="font-semibold text-gray-800">
                  {prop.proposedPriceLow.toLocaleString()} -{" "}
                  {prop.proposedPriceHigh.toLocaleString()} {prop.currency}
                </span>
              </div>
              <div className="bg-gray-50 p-2 rounded border border-gray-100">
                ...
                <span className="font-semibold text-gray-800">
                  {prop.shippingMode} |{" "}
                  {prop.escrowRequired ? "Escrow Protected" : "Open Terms"}
                </span>
              </div>
            </div>
