...
export function LiquidityMatchPanel() {
  ...
  if (loading)
    return (
      <div className="p-4 bg-white rounded-xl border border-gray-100 shadow-sm animate-pulse h-32"></div>
    );
  ...
  if (matches.length === 0) {
    return (
      <div className="p-6 bg-white rounded-xl border border-gray-100 flex items-center justify-center text-gray-500">
        <span>
          No automated trade matches proposed yet. System is scanning...
        </span>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm flex flex-col">
      <div className="p-4 border-b border-gray-100 flex items-center justify-between">
        ...
        <span className="text-xs bg-green-100 text-green-700 px-2 h-6 rounded flex items-center">
      ...
      <div className="divide-y divide-gray-50 flex flex-col gap-2 p-4">
        {matches?.map((match) => (
          ...
          >
            <div className="flex justify-between items-start">
              <div className="flex flex-col">
                <span className="font-semibold text-gray-900 leading-tight">
                  Match for {match.categoryId || "General"}
                </span>
                <span className="text-sm text-gray-500 mt-1 flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-blue-500"></span>
                  Buyer: {match.buyerTenantId.substring(0, 8)} | Seller:{" "}
                  {match.sellerTenantId.substring(0, 8)}
                </span>
              </div>
              <div className="flex flex-col items-end gap-1">
                <span className="font-bold text-gray-900 text-lg">
                  Score: {Math.round(match.finalMatchScore)}
                </span>
            ...
            {match.explainJson &&
              match.explainJson.drivers &&
              match.explainJson.drivers.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-2">
                  {match.explainJson.drivers?.map(
                  ...
                  {match.explainJson.riskFlags?.map(
                    (flag: string, idx: number) => (
                      <span
                        key={`risk-${idx}`}
                        className="px-2 py-1 bg-red-50 text-red-700 text-xs rounded border border-red-100"
                      >
                        {flag.replace(/_/g, " ")}
                      </span>
                    ),
