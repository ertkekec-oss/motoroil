          ...
          >
            <div className="flex flex-col">
              <span className="text-sm font-semibold text-gray-900">
                {opp.opportunityType === "SUPPLY_SURPLUS"
                  ? "Excess Supply"
                  : "Demand Shortage"}{" "}
                : {opp.categoryId || "General"}
              </span>
              <span className="text-xs text-gray-500 mt-1">
                Volume Score: {opp.liquidityVolumeScore.toFixed(1)} | Location:{" "}
                {opp.regionCode || "N/A"}
              </span>
            </div>
            <div className="flex flex-col items-end">
              <span
                className={`text-xs px-2 py-1 rounded-lg ${opp.status === "RANKED" ? "bg-purple-100 text-purple-700" : "bg-gray-100 text-gray-700"}`}
              >
                {opp.status}
              </span>
            </div>
