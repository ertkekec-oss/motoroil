...
export function CategoryLiquidityHeatmap() {
  ...
  if (loading)
    return (
      <div className="p-4 bg-gray-50 animate-pulse h-24 rounded-lg">
        Loading Heatmap...
      </div>
    );
  ...
  if (snapshots.length === 0) {
    return (
      <div className="p-4 bg-white border border-gray-100 rounded-lg text-gray-500 text-sm">
        No categorical heatmap data yet. Run nightly snapshot generator.
      </div>
    );
  }
          ...
          <div key={snap.id} className="group">
            <div className="flex justify-between items-center text-sm font-medium mb-1.5">
              <span className="text-gray-900">
                {snap.categoryId || "General"}
              </span>
              <span className="text-gray-500 font-normal">
                Gap Score: {snap.liquidityGapScore}
              </span>
            </div>
