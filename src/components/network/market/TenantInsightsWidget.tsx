...
const Button = (p: any) => (
  <button
    className={`px-3 py-1 bg-blue-600 text-white rounded text-sm ${p.className || ""}`}
    onClick={p.onClick}
  >
    {p.children}
  </button>
...
export function TenantInsightsWidget({
  insights,
  isLoading,
  onActionClick,
}: TenantInsightProps) {
      ...
      <Card className="border-border shadow-sm">
        ...
        <CardContent>
          <div className="flex flex-col items-center justify-center py-6 text-center text-muted-foreground">
            ...
            <p className="text-sm">
              Bugün sizin için aktif bir öneri bulunmuyor.
            </p>
          </div>
        </CardContent>
      </Card>
    ...
    <Card className="border-border bg-slate-50 shadow-sm">
      ...
      <CardContent className="space-y-3">
        {insights?.map((insight) => (
          <div
            key={insight.id}
            className="flex flex-col gap-2 p-3 bg-white border border-emerald-100 rounded-lg shadow-sm"
          >
            <div className="flex items-center gap-2">
              <Badge
                variant={insight.priority === 1 ? "destructive" : "secondary"}
