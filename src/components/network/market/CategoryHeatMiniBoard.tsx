...
export function CategoryHeatMiniBoard({ categories, isLoading }: CategoryHeatProps) {
    if (isLoading) {
        return (
            <Card className="border-border shadow-sm">
                <CardHeader>
                    <CardTitle className="text-base font-semibold">Pazar Isı Haritası</CardTitle>
                </CardHeader>
                    ...
                    {categories.slice(0, 5).map((cat, index) => (
                        <div key={index} className="flex justify-between items-center p-2 rounded hover:bg-muted/50 transition">
                            ...
                            <div className="flex items-center gap-3">
                                <span className="text-xs font-semibold text-muted-foreground">{cat.maxIntensity}%</span>
                                {cat.trendDirection === 'UP' && <TrendingUp className="h-4 w-4 text-emerald-500" />}
                                {cat.trendDirection === 'DOWN' && <TrendingDown className="h-4 w-4 text-rose-500" />}
                                {cat.trendDirection === 'FLAT' && <Minus className="h-4 w-4 text-slate-500" />}
                            </div>
                        </div>
                    ))}
                </div>
