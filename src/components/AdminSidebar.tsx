...
export function AdminSidebar({ userRole }: { userRole: string }) {
  ...
  const NavItem = ({
    ...
    return (
      <Link
        href={href}
        className={`flex items-center gap-3 px-3 py-2 text-sm rounded-md transition-colors ${isActive ? "bg-blue-600/10 text-blue-400 font-medium" : "text-slate-400 hover:text-slate-100 hover:bg-slate-800"}`}
      >
  ...
  const NavGroup = ({
    ...
    const isChildActive = React.Children.toArray(children).some(
      (child: any) => {
        if (child.props.href && pathname.startsWith(child.props.href))
          return true;
        return false;
      },
    );
      ...
      <div className="space-y-1 mb-6">
        ...
        {isSuper && (
          ...
          >
            ...
            />
            <NavItem
              href="/admin/help/knowledge-base"
              icon={FileText}
              label="Bilgi Bankası (Hub)"
            />
            <NavItem
      ...
      <div className="space-y-1 mb-6">
        ...
        <NavGroup title="PERİODYA HUB" icon={Network} groupKey="hub_main">
          {(isSuper || isFinance) && (
            ...
            >
              <NavItem
                ...
                icon={CreditCard}
                label="Kurumsal Fatura Merkezi"
              />
              <NavItem
                href="/admin/payments-escrow/policies"
                icon={FileText}
                label="Escrow Politikaları"
              />
              <NavItem
                href="/admin/payments-escrow/commissions"
                icon={Percent}
          ...
          {(isSuper || isGrowth || isFinance) && (
            ...
            >
              <NavItem
                href="/admin/growth/boost-rules"
                icon={TrendingUp}
                label="Boost Kuralları"
              />
              <NavItem
                href="/admin/growth/billing-health"
                icon={Activity}
              ...
              <NavItem
                ...
                icon={LayoutDashboard}
                label="Gelir İstatislikleri"
              />
              <NavItem
                href="/admin/growth/audit"
                icon={FileText}
                label="Büyüme Denetimi"
              />
              <NavItem
                href="/admin/growth/subscriptions"
                icon={Users}
                label="Hub Abonelikleri"
              />
            </NavGroup>
          )}
          ...
          {(isSuper || isRisk) && (
            ...
            >
              <NavItem
                href="/admin/disputes"
                icon={AlertTriangle}
                label="Uyuşmazlık Kuyruğu"
              />
              <NavItem
                href="/admin/trust-monitor"
                icon={Activity}
                label="Güven Monitörü"
              />
              <NavItem
                href="/admin/disputes/policies"
                icon={FileText}
                label="Çözüm Politikaları"
              />
            </NavGroup>
          )}
          ...
          {isSuper && (
            ...
            >
              ...
              <NavItem
                ...
                icon={DatabaseZap}
                label="Fintech Routing"
              />
              <NavItem
                href="/admin/ops/limits"
                icon={Activity}
                label="API Limitleri"
              />
              <NavItem
                href="/admin/ops/providers"
                icon={Server}
                label="Platform Altyapısı"
              />
              <NavItem
                href="/admin/ops/ledgers"
                icon={DatabaseZap}
                label="Sistem Defteri"
              />
            </NavGroup>
          )}
        </NavGroup>
        ...
        {isSuper && (
          ...
          >
            <NavItem
              ...
              icon={Activity}
              label="B2B Sipariş Kuyruğu"
            />
            <NavItem
              href="/admin/b2b/refunds"
              icon={ArrowRightLeft}
              label="B2B İade ve Uyuşmazlıklar"
            />
            <NavItem
              href="/admin/b2b/policies"
              icon={ShieldAlert}
              label="B2B Risk Politikaları"
            />
          </NavGroup>
        )}
      </div>
      ...
      {isSuper && (
        <div className="space-y-1 mb-6">
          ...
          <NavGroup
            title="İmza Yönetimi"
            icon={PenTool}
            groupKey="admin_signatures"
          >
            <NavItem
              href="/admin/signatures"
              icon={LayoutDashboard}
              label="İmza Dashboard"
            />
            <NavItem
            ...
            <NavItem
              ...
              icon={ShieldCheck}
              label="Kurallar (Policy)"
            />
            <NavItem
              href="/admin/signatures/webhooks"
              icon={Activity}
              label="Webhook & Callback"
            />
            <NavItem
              href="/admin/signatures/audit"
              icon={Search}
              label="İmza Denetimi"
            />
          </NavGroup>
          ...
          <NavGroup
            ...
            icon={Handshake}
            groupKey="admin_recon"
          >
            <NavItem
              href="/admin/reconciliation"
              icon={LayoutDashboard}
              label="Mutabakat Dashboard"
            />
            <NavItem
              href="/admin/reconciliation/templates"
              icon={FileText}
              label="Şablonlar"
            />
            <NavItem
              href="/admin/reconciliation/policies"
              icon={ShieldCheck}
              label="Kurallar (Policy)"
            />
            <NavItem
              href="/admin/reconciliation/jobs"
              icon={Inbox}
              label="Gönderim İşleri"
            />
            <NavItem
              href="/admin/reconciliation/disputes"
              icon={AlertTriangle}
              label="Uyuşmazlıklar"
            />
            <NavItem
              href="/admin/reconciliation/audit"
              icon={Search}
