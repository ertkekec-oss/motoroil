...
export default function EditCampaign(props: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        const fetchCampaign = async () => {
            try {
                ...
                const current = (data || []).find((c: any) => c.id === params.id);
                if (current) {
                    setFormData({
                        name: current.name || "",
                        campaignType: current.campaignType || "PERCENT_DISCOUNT",
                        channels: current.channels && current.channels.length ? current.channels : ["GLOBAL"],
                        priority: current.priority || 0,
                        stackingRule: current.stackingRule || "STACKABLE",
                        discountRate: current.campaignType === "LOYALTY_POINTS" && current.pointsRate ? String(current.pointsRate * 100) : (current.discountRate || ""),
                        minOrderAmount: current.minOrderAmount || "",
                        validFrom: current.validFrom ? new Date(current.validFrom).toISOString().split('T')[0] : "",
                        validUntil: current.validUntil ? new Date(current.validUntil).toISOString().split('T')[0] : "",
                        description: current.description || "",
                        buyQuantity: current.conditions?.buyQuantity || 1,
                        rewardQuantity: current.conditions?.rewardQuantity || 1,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                showSuccess("Başarılı", "Kampanya güncellendi.");
                router.push("/campaigns/active");
            } else {
    ...
    if (pageLoading) return <div className="p-8"><Loader2 className="w-8 h-8 animate-spin mx-auto text-blue-500" /></div>;
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden max-w-4xl mx-auto">
            <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
                <div className="flex items-center gap-4">
                    <button type="button" onClick={() => router.back()} className="p-2 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-xl transition-colors">
                        <MoveLeft className="w-5 h-5 text-slate-600 dark:text-slate-300" />
                    </button>
                    <div>
                        <h3 className="font-bold text-slate-800 dark:text-white text-lg">Kampanyayı Düzenle</h3>
                        <p className="text-xs text-slate-500 mt-0.5">Campaign Engine V2</p>
                    </div>
                </div>
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="col-span-1 md:col-span-2">
                    ...
                    <input required value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} type="text" className="w-full border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 bg-slate-50 dark:bg-slate-900/50 outline-none focus:ring-2 focus:ring-indigo-500/50" />
                </div>
