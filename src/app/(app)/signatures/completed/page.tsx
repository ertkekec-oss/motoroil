            ...
            <div style={{ maxWidth: '1200px', margin: '0 auto', width: '100%' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
                    <div>
                        ...
                        <p style={{ color: 'var(--text-muted)', fontSize: '14px', margin: 0 }}>Tüm imzacıların onayından başarıyla geçmiş zarflar.</p>
                    </div>
                </div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                        <thead>
                            <tr style={{ background: 'rgba(0,0,0,0.2)', borderBottom: '1px solid var(--border-color)' }}>
                                <th style={{ padding: '16px 24px', fontSize: '12px', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Tamamlanma Tarihi</th>
                                <th style={{ padding: '16px 24px', fontSize: '12px', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Zarf Başlığı</th>
                                <th style={{ padding: '16px 24px', fontSize: '12px', color: 'var(--text-muted)', textTransform: 'uppercase' }}>İmzacı Sayısı</th>
                        ...
                        <tbody>
                            {completedEnvelopes.length === 0 ? (
                                <tr>
                                    <td colSpan={4} style={{ padding: '32px', textAlign: 'center', color: 'var(--text-muted)' }}>Henüz tamamlanmış imza bulunmuyor.</td>
                                </tr>
                                ...
                                <tr key={env.id} className="hover:bg-slate-800/20" style={{ borderBottom: '1px solid var(--border-color)' }}>
                                    ...
                                    <td style={{ padding: '16px 24px', fontSize: '13px' }}>
                                        <span style={{ color: '#10b981', fontWeight: '800' }}>✔ {env.recipients.length} Kişi</span>
                                    </td>
                                    <td style={{ padding: '16px 24px', textAlign: 'right' }}>
                                        <Link href={`/signatures/envelopes/${env.id}`} style={{ padding: '6px 16px', background: 'rgba(59,130,246,0.1)', color: '#3b82f6', borderRadius: '8px', cursor: 'pointer', fontSize: '12px', fontWeight: '600', textDecoration: 'none' }} className="hover:bg-blue-500/20 transition-all">
