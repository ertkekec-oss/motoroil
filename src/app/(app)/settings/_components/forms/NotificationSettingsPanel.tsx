...
function ERPSwitch({ checked, onChange, label, description }: { checked: boolean, onChange: (e: React.ChangeEvent<HTMLInputElement>) => void, label: string, description?: string }) {
    return (
        <label className="flex items-start justify-between cursor-pointer p-4 hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-0">
            <div className="flex-1 pr-8">
                <span className="block text-[14px] font-semibold text-slate-900">{label}</span>
                {description && <span className="block mt-1 text-[13px] text-slate-500 leading-relaxed">{description}</span>}
            </div>
...
function ERPBlock({ title, description, children }: { title?: string, description?: string, children: React.ReactNode }) {
    return (
        <div className="bg-white border border-slate-200 rounded-2xl shadow-[0px_1px_2px_rgba(0,0,0,0.02)] mb-6 overflow-hidden">
            {(title || description) && (
    ...
    return (
        <div className="max-w-4xl animate-in fade-in duration-300">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <div>
                    <h2 className="text-[24px] font-semibold text-slate-900 tracking-tight">Olay İkaz / Geri Bildirim Ağı</h2>
                    <p className="text-[14px] text-slate-500 mt-1">Hayati işlem döngülerinde ve risk izleme ekranlarında yöneticinin alacağı sistem trigger'ları.</p>
                </div>
