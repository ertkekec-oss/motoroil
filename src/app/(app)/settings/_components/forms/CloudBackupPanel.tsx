        ...
        <div className="w-full bg-white border border-slate-200 rounded-2xl shadow-[0px_1px_2px_rgba(0,0,0,0.02)] overflow-hidden mb-6 flex divide-x divide-slate-100">
            {items.map((it, idx) => (
                <div key={idx} className="flex-1 p-5">
                    <div className="text-[20px] font-semibold text-slate-900 tracking-tight">{it.val}</div>
                    <p className="text-[12px] font-medium text-slate-500 uppercase tracking-widest mt-1.5">{it.label}</p>
                </div>
            ))}
        </div>
    );
...
export default function CloudBackupPanel(props: any) {
    return (
        <div className="max-w-5xl animate-in fade-in duration-300">
            {/* Header */}
            <div className="mb-8">
                <h2 className="text-[24px] font-semibold text-slate-900 tracking-tight">Sistem Yedekleme & İş Sürekliliği</h2>
                <p className="text-[14px] text-slate-500 mt-1">Sistem veritabanını güvence altına alıp, anlık veri kurtarma noktaları (snapshot) yaratmanızı sağlar.</p>
            </div>
            ...
            {/* Kotanın Grafiği */}
            <div className="mb-8 rounded-xl bg-white border border-slate-200 p-4 shadow-[0px_1px_2px_rgba(0,0,0,0.02)] flex items-center gap-4">
                <div className="flex-1 w-full bg-slate-100 rounded-full h-2 shadow-inner overflow-hidden">
                    <div className="bg-slate-900 rounded-full h-full" style={{ width: '12%' }} />
                </div>
                <div className="text-[12px] font-semibold text-slate-700 tracking-wider">
                    %12 DOLU
                </div>
            </div>
            ...
            <ERPBlock title="Yedekleme ve Geri Yükleme Yönetimi" description="Güvenlik politikası gereği yedekleme paketleri şifrelenmiş formda teslim edilir.">
                <div className="space-y-4">
                    {/* Manuel Snapshot */}
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between p-5 border border-slate-200 rounded-xl bg-slate-50 relative group transition-colors hover:bg-white hover:shadow-sm">
                        <div className="flex items-start gap-3">
                            <div className="w-10 h-10 bg-white border border-slate-200 rounded-lg flex items-center justify-center shadow-sm text-lg mt-0.5 shrink-0">
                                🛡️
                            </div>
                            <div>
                                <h4 className="text-[15px] font-semibold text-slate-900">Anlık Sistem Snapshot</h4>
                                <p className="text-[13px] text-slate-600 mt-0.5 max-w-sm">Riskli majör konfigürasyonlardan önce sistemin tam state'ini kayıt altına alın.</p>
                            </div>
                        </div>
                        <button
                            onClick={() => { /* unchanged logic */ }}
                            className="mt-4 sm:mt-0 px-5 h-10 bg-slate-900 text-white rounded-lg text-[13px] font-medium shadow-sm hover:bg-slate-800 transition-colors shrink-0"
                        >
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between p-5 border border-slate-200 rounded-xl bg-slate-50 relative group transition-colors hover:bg-white hover:shadow-sm">
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <h4 className="text-[15px] font-semibold text-slate-900">SQL Data İndirme</h4>
                                <p className="text-[13px] text-slate-600 mt-0.5 max-w-sm">Tüm veritabanı JSON/SQL yapılarıyla birlikte arşiv paketi olarak indirin.</p>
                            </div>
                        </div>
                        <button
                            onClick={() => { /* unchanged logic */ }}
                            className="mt-4 sm:mt-0 px-5 h-10 bg-white border border-slate-300 text-slate-700 rounded-lg text-[13px] font-medium shadow-sm hover:bg-slate-50 transition-colors shrink-0 flex items-center gap-2"
                        >
