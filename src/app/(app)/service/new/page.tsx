...
import { ArrowLeft, Search, Shield, X, Package, Check, Info, Calendar, UserPlus, FileText } from 'lucide-react';
...
function ServiceAcceptanceContent() {
    ...
    const inputStyle = `h-10 px-3 rounded-lg border text-sm transition-all focus:ring-2 focus:ring-blue-500/20 outline-none w-full ${bgMain} ${borderMain} ${textMain}`;
    const labelStyle = `text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-1.5 block`;
        ...
        <div className={`min-h-screen ${isLight ? 'bg-slate-50' : 'bg-[#020617]'} p-6 font-sans`}>
            ...
            {warrantyModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-950/40 backdrop-blur-sm">
                    <div className={`w-full max-w-md rounded-xl border p-6 shadow-2xl ${bgMain} ${borderMain}`}>
                        ...
                        <div className="space-y-2 mb-8">
                            {customerWarranties.map(w => (
                                <button key={w.id} onClick={() => { setFormData({...formData, brand: w.productName, serialNumber: w.serialNo}); setWarrantyModalOpen(false); }} className={`w-full text-left p-4 rounded-lg border hover:border-blue-500 transition-all ${bgMuted} ${borderMain}`}>
                                    <div className={`font-semibold ${textMain}`}>{w.productName}</div>
                                    <div className="text-xs text-slate-500 mt-1">S/N: {w.serialNo} • {w.endDate} bitiş</div>
                                </button>
                            ))}
                        </div>
                        <button onClick={() => setWarrantyModalOpen(false)} className={`w-full py-2.5 rounded-lg text-sm font-medium ${isLight ? 'text-slate-600 hover:bg-slate-100' : 'text-slate-400 hover:bg-slate-800'}`}>Kapat</button>
                    </div>
                </div>
            )}
            ...
            <div className="max-w-[1400px] mx-auto space-y-6">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <button onClick={() => router.back()} className={`w-9 h-9 rounded-lg border flex items-center justify-center ${bgMain} ${borderMain} ${textMuted} hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors`}><ArrowLeft size={18} /></button>
                        <div>
                            <h1 className={`text-xl font-semibold tracking-tight ${textMain}`}>Yeni Servis Kabulü</h1>
                            <p className={`text-sm ${textMuted}`}>İş emri giriş ve müşteri kayıt terminali.</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900">
                        <Calendar size={14} className={textMuted} />
                        <span className={`text-xs font-medium ${textMuted}`}>{new Date().toLocaleDateString('tr-TR')}</span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
                    ...
                    <div className="lg:col-span-3 space-y-6">
                        {/* BASIC INFO SECTION */}
                        <div className={`p-6 rounded-xl border ${bgMain} ${borderMain}`}>
                            <div className="mb-6">
                                <label className={labelStyle}>Hizmet Kategorisi</label>
                                <div className="flex flex-wrap gap-2">
                                    {['Motosiklet', 'Bisiklet', 'Beyaz Eşya', 'Elektronik'].map(t => (
                                        <button key={t} onClick={() => setVehicleType(t)} className={`h-8 px-4 rounded-full text-xs font-semibold transition-all border ${vehicleType === t ? 'bg-blue-600 border-blue-600 text-white' : 'bg-slate-100 dark:bg-slate-800 border-transparent text-slate-500 hover:border-slate-300'}`}>{t}</button>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                <div className="md:col-span-2 relative">
                                    ...
                                    <div className="relative">
                                        ...
                                        <Search size={16} className={`absolute right-3 top-1/2 -translate-y-1/2 ${textMuted} opacity-40`} />
                                    </div>
                                    {isNewCustomer && searchQuery.length > 2 && <div className="absolute right-10 top-1/2 -translate-y-1/2 px-2 py-0.5 rounded-md bg-amber-500/10 text-amber-600 text-[10px] font-bold uppercase tracking-wider border border-amber-500/10">YENİ ÜYE</div>}
                                    {showSuggestions && filteredCustomers.length > 0 && (
                                        <div className={`absolute left-0 right-0 top-full mt-2 rounded-lg border shadow-xl z-50 overflow-hidden ${bgMain} ${borderMain}`}>
                                            {filteredCustomers.map(c => (
                                                <button key={c.id} onClick={() => selectCustomer(c)} className="w-full text-left p-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 flex justify-between items-center transition-colors">
                                                    <span className={`font-semibold ${textMain}`}>{c.name}</span>
                                                    <span className={`text-xs ${textMuted}`}>{c.phone}</span>
                                                </button>
                                            ))}
                                        </div>
                                    )}
                            ...
                            <div className={`p-6 rounded-xl border ${bgMain} ${borderMain}`}>
                                ...
                                <div className="grid grid-cols-1 gap-2">
                                    {["Genel Temizlik", "Lastik/Ayak Kontrolü", "Yağ Seviyesi", "Fren Testi"].map((item, i) => (
                                        <button key={i} onClick={() => setFormData(p => ({...p, checklist: {...p.checklist, [item]: !p.checklist[item]}}))} className={`flex items-center gap-3 p-3 rounded-lg border transition-all text-left ${formData.checklist[item] ? 'bg-blue-50/50 border-blue-500 text-blue-700' : 'bg-transparent border-slate-100 dark:border-slate-800 text-slate-500 hover:border-slate-200'}`}>
                                            <div className={`w-4 h-4 rounded border flex items-center justify-center transition-all ${formData.checklist[item] ? 'bg-blue-600 border-blue-600 text-white' : 'border-slate-300'}`}>{formData.checklist[item] && <Check size={10} strokeWidth={4} />}</div>
                            ...
                            <div className={`p-6 rounded-xl border ${bgMain} ${borderMain}`}>
                                ...
                                <div className="relative mb-4">
                                    ...
                                    {showProductSuggestions && filteredProducts.length > 0 && (
                                        <div className={`absolute left-0 right-0 top-full mt-1 border rounded-lg shadow-lg z-50 overflow-hidden ${bgMain} ${borderMain}`}>
                                            {filteredProducts.map(p => (
                                                <button key={p.id} onClick={() => addPart(p)} className="w-full text-left p-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 flex justify-between items-center">
                                                    <span className={textMain}>{p.name}</span>
                                                    <span className="text-blue-500 font-bold">₺{p.price}</span>
                                                </button>
                                            ))}
                                        </div>
                                    )}
                                    ...
                                    {selectedParts.map(p => (
                                        <div key={p.id} className={`flex items-center justify-between p-2 rounded-lg ${bgMuted}`}>
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <button onClick={() => setSelectedParts(prev => prev.filter(x => x.id !== p.id))} className="text-slate-400 hover:text-red-500"><X size={14}/></button>
                                            </div>
                                        </div>
                                    ))}
                                    <div className={`flex items-center justify-between p-2 rounded-lg border-2 border-dashed ${isLight ? 'border-slate-100 bg-slate-50/30' : 'border-slate-800 bg-white/5'}`}>
                                        <div className="flex flex-col">
                                            <span className={`text-xs font-bold ${textMain}`}>Hizmet Bedeli</span>
                                            <span className="text-[10px] text-blue-500 uppercase tracking-tighter">Standart Bakım</span>
                                        </div>
                    ...
                    <div className="space-y-6">
                        <div className={`rounded-xl border p-6 flex flex-col items-center ${bgMain} ${borderMain}`}>
                            ...
                            <div className="w-full pt-6 space-y-3">
                                <button onClick={() => handleSave('İşlemde')} className="w-full h-11 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold transition-all">Atölyeye Al</button>
                                <button onClick={() => handleSave('Tamamlandı')} className={`w-full h-11 rounded-lg border text-sm font-semibold transition-all ${isLight ? 'bg-slate-50 border-slate-200 text-slate-700 hover:bg-slate-100' : 'bg-white/5 border-white/10 text-slate-300 hover:bg-white/10'}`}>Bitti & Ödeme Al</button>
                                <button onClick={() => handleSave('Beklemede')} className={`w-full h-11 rounded-lg border text-sm font-semibold transition-all ${isLight ? 'bg-slate-50 border-slate-200 text-slate-700 hover:bg-slate-100' : 'bg-white/5 border-white/10 text-slate-300 hover:bg-white/10'}`}>Randevu Yap</button>
                            </div>
                        </div>
                        
                        <div className={`p-4 rounded-xl border flex items-start gap-4 ${bgMuted} ${borderMain}`}>
                            <Info size={16} className="text-blue-500 shrink-0 mt-0.5" />
                            <p className="text-xs text-slate-500 leading-relaxed">Randevu girilen kayıtlar teknisyen takviminde otomatik olarak ayırılır.</p>
                        </div>
                    </div>
                </div>
            </div>
...
export default function ServiceAcceptancePage() {
    return (
        <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-[#020617] text-slate-400">Yükleniyor...</div>}>
