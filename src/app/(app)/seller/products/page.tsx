        ...
        <div className="bg-slate-50 dark:bg-[#0f172a] dark:bg-[#020617] dark:bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] dark:from-slate-900/40 dark:via-[#020617] dark:to-[#020617] min-h-screen pb-16 w-full font-sans">
            <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-in fade-in duration-300">
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-5 mb-8">
                    <div>
                        <h1 className="text-2xl font-semibold text-slate-900 dark:text-white dark:text-white dark:text-white tracking-tight mb-1">
                            Katalog (Ürünlerim) / B2B Mağaza
                        </h1>
                        <p className="text-sm text-slate-600 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300">
                ...
                <div className="bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-white/10 dark:border-white/10 dark:border-white/10 rounded-2xl shadow-sm overflow-hidden">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left table-auto">
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-white/5 dark:divide-white/5 dark:divide-white/5 text-[14px]">
                                {products.length === 0 ? (
                                    <tr>
                                        <td colSpan={7} className="px-6 py-16 text-center text-slate-500 dark:text-slate-400 dark:text-slate-400 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-400">
                                            <div className="text-3xl mb-4">📦</div>
                                            <p className="text-[15px] font-semibold text-slate-900 dark:text-white dark:text-white dark:text-white">ERP Envanteri Boş</p>
                                            <p className="text-[13px] text-slate-500 dark:text-slate-400 dark:text-slate-400 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-400 max-w-sm mx-auto mt-1">Şu anda lokal veritabanınızda eşlenik bir ürün bulunmuyor.</p>
                                        </td>
                                    </tr>
                                ) : (
                                    products.map(p => {
                                        ...
                                        return (
                                            <tr key={p.id} className="hover:bg-slate-50 dark:bg-[#0f172a] transition-colors group">
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="font-semibold text-slate-900 dark:text-white dark:text-white dark:text-white">{p.name}</div>
                                                </td>

                                                <td className="px-6 py-4 text-center">
                                                    <span className="font-bold text-slate-700 dark:text-slate-200 dark:text-slate-200 dark:text-slate-200 font-mono text-[14px]">
                                                        {p.stock}
                                                    </span>
                                                </td>
                                                ...
                                                <td className="px-6 py-4 text-right">
                                                    {isListed ? (
                                                        <div className="font-mono font-bold text-[15px] text-slate-900 dark:text-white dark:text-white dark:text-white">
                                                            {Number(listing.price).toLocaleString('tr-TR', { minimumFractionDigits: 2 })}
                                                            <span className="text-slate-400 dark:text-slate-500 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-400 text-[12px] ml-1">₺</span>
                                                        </div>
                                                    ) : (
