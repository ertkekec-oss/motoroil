...
export default function BoostPage() {
        ...
        fetch("/api/seller/boost")
            ...
            .then(data => {
                if (data && data.subscription) {
                    setBoostData({
                        planName: data.subscription.planName || t('boost.unknownPlan'),
    ...
    return (
        <div className="bg-slate-50  min-h-screen pb-16 w-full font-sans">
            <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-in fade-in duration-300">
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-5 mb-6">
                    <div className="flex-1">
                        ...
                        <p className="text-[13px] text-slate-500  mt-1.5 max-w-4xl">{t('boost.desc')}</p>
                    </div>
            ...
            {loading ? (
                <div className="bg-white  p-6 rounded-xl border border-slate-200  shadow-sm text-center py-12 text-slate-500 ">{t('boost.loading')}</div>
            ) : !boostData ? (
                <div className="bg-white  p-6 rounded-xl border border-slate-200  shadow-sm text-center py-12 text-slate-500 ">{t('boost.noSub')}</div>
            ) : (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="bg-gradient-to-br from-indigo-900 via-indigo-800 to-indigo-900 rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                            <div className="flex justify-between items-start mb-6">
                                <div>
                                    <p className="text-indigo-200 font-medium uppercase tracking-wider text-xs mb-1">{t('boost.currentPlan')}</p>
                                ...
                                <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide border ${boostData.status === 'ACTIVE' ? 'bg-indigo-800/50 text-indigo-100 border-indigo-400/30' : 'bg-red-500/20 text-red-200 border-red-500/50'}`}>
                                    {boostData.status === 'ACTIVE' ? t('boost.active') : t('boost.suspended')}
                                </div>
                            </div>

                            <div className="mb-8">
                                <span className="text-4xl font-bold">{formatMoney(boostData.price)}</span>
                                <span className="text-indigo-200 font-medium text-sm ml-1">{t('boost.perMonth')}</span>
                            </div>

                            <div className="bg-white  rounded-2xl p-5  border border-white/10 mb-6">
                                <div className="flex justify-between text-sm mb-2 font-medium">
                                    <span className="text-indigo-100">{t('boost.quotaUsage')}</span>
                                ...
                                <p className="text-xs text-indigo-300">{t('boost.renewalDate')}: <span className="text-indigo-100 font-semibold">{formatDate(boostData.renewalDate)}</span></p>
                            </div>
                            ...
                            <Link href="/settings">
                                <button className="w-full bg-white  text-indigo-900 font-bold py-3 rounded-xl hover:bg-indigo-50 transition-colors shadow-sm shadow-indigo-900/50 relative overflow-hidden group">
                                    <span className="relative z-10">{t('boost.changePlan')}</span>
                                </button>
                            </Link>
                        </div>
                    </div>
                    ...
                    <div className="bg-white  rounded-3xl p-8 border border-slate-200  shadow-sm flex flex-col justify-center">
                        <h3 className="text-xl font-bold text-slate-900  mb-6">{t('boost.howItWorks')}</h3>
                        ...
                        <div className="space-y-6">
                            <div className="flex items-start">
                                ...
                                <div className="ml-4">
                                    <h4 className="text-base font-bold text-slate-800">{t('boost.sponsoredViews')}</h4>
                                    <p className="text-sm text-slate-600  mt-1">{t('boost.sponsoredDesc')}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-start">
                                ...
                                <div className="ml-4">
                                    <h4 className="text-base font-bold text-slate-800">{t('boost.perfTracking')}</h4>
                                    <p className="text-sm text-slate-600  mt-1">{t('boost.perfDesc')}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-start">
                                ...
                                <div className="ml-4">
                                    <h4 className="text-base font-bold text-slate-800">{t('boost.billingPayment')}</h4>
                                    <p className="text-sm text-slate-600  mt-1">{t('boost.billingDesc')}</p>
                                </div>
                            </div>
                        </div>
                    </div>
