        ...
        <div className="bg-slate-50  min-h-screen pb-16 w-full font-sans">
            <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-in fade-in duration-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div className="bg-white  px-5 py-4 rounded-xl border border-slate-200  shadow-sm flex items-center gap-4">
                        ...
                        <div className="flex-1">
                            ...
                            <div className="text-lg font-black text-slate-900  leading-none">{mappingStats.total} <span className="text-xs font-semibold text-slate-500 ml-1">Kayıt</span></div>
                        </div>
                    </div>

                    <div className="bg-emerald-50/50  px-5 py-4 rounded-xl border border-emerald-100  shadow-sm flex items-center gap-4">
                        <div className="w-10 h-10 bg-emerald-100  rounded-lg flex items-center justify-center text-emerald-600  shrink-0">
                            <CheckCircle2 className="w-5 h-5" />
                        </div>
                        <div className="flex-1">
                            <div className="text-[10px] font-bold text-emerald-600/70  uppercase tracking-widest mb-0.5">Global'e Eşleşen</div>
                            <div className="text-lg font-black text-emerald-700  leading-none">{mappingStats.mapped} <span className="text-xs font-semibold text-emerald-600/70 ml-1">Kayıt</span></div>
                        </div>
                    </div>

                    <div className="bg-rose-50/50  px-5 py-4 rounded-xl border border-rose-100  shadow-sm flex items-center gap-4">
                        <div className="w-10 h-10 bg-rose-100  rounded-lg flex items-center justify-center text-rose-600  shrink-0">
                            <AlertCircle className="w-5 h-5" />
                        </div>
                        <div className="flex-1">
                            <div className="text-[10px] font-bold text-rose-600/70  uppercase tracking-widest mb-0.5">Eşleşme Bekleyen</div>
                            <div className="text-lg font-black text-rose-700  leading-none">{mappingStats.pending} <span className="text-xs font-semibold text-rose-600/70 ml-1">Kayıt</span></div>
                        </div>
                    </div>
                </div>

                <div className="mb-6 bg-slate-900  border border-indigo-500/30 rounded-xl px-4 py-3 shadow-md flex items-center justify-between gap-4 w-full">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div className="p-4 flex-1 overflow-y-auto">
                            {localCategories.length === 0 ? (
                                <div className="text-center py-20 opacity-60">
                                    ...
                                    <h3 className="text-sm font-bold text-slate-700 ">Lokal Kategori Bulunamadı</h3>
                                    ...
                                    {localCategories.map((cat: any) => {
                                        ...
                                        <div 
                                            ...
                                            className={`flex items-center justify-between p-3.5 rounded-xl border transition-all shadow-sm group ${
                                                cat.globalCategory 
                                                    ? 'bg-slate-50/50  border-slate-100  opacity-80 cursor-default'
                                                    : isSelected 
                                                        ? 'bg-indigo-50  border-indigo-400  ring-2 ring-indigo-500/20 cursor-pointer scale-[1.01]' 
                                                        : 'bg-white  border-slate-200  hover:border-indigo-300 :border-indigo-500/30 cursor-pointer'
                                            }`}
                                        >
                                            <div className="flex items-center gap-3">
                                                <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-medium border ${
                                                    isSelected ? 'bg-indigo-100  text-indigo-600 border-indigo-200 ' : 'bg-slate-50  text-slate-400 group-hover:text-indigo-500 border-slate-100 '
                                                }`}>
                                                    🏷️
                                                </div>
                                                <div>
                                                    <div className={`text-[13px] font-bold transition-colors ${isSelected ? 'text-indigo-700 ' : 'text-slate-700  group-hover:text-indigo-600 :text-indigo-400'}`}>{cat.name}</div>
                                                    <div className="text-[11px] text-slate-500 font-medium">{cat._count?.id || cat.count || 0} Ürün bağlandı</div>
                                                </div>
                                            </div>
                                            {cat.globalCategory ? (
                                                <button disabled className="h-8 px-3 rounded text-[11px] font-bold tracking-widest uppercase bg-emerald-50  text-emerald-600 border border-emerald-100  cursor-default">
                                                    Eşleşti 
                                                </button>
                                            ) : (
                                                <button className={`h-8 px-3 rounded text-[11px] font-bold tracking-widest uppercase transition-colors border ${
                                                    isSelected ? 'bg-indigo-600 text-white border-indigo-600 shadow-md scale-105' : 'bg-indigo-50  text-indigo-600 border-indigo-100  group-hover:bg-indigo-600 group-hover:text-white'
                                                }`}>
                    ...
                    <div className="bg-slate-50  rounded-2xl border border-slate-200  shadow-inner overflow-hidden flex flex-col h-[600px] select-none">
                        <div className="px-5 py-4 border-b border-slate-200  bg-white  flex items-center justify-between shadow-sm z-10">
                            <div>
                                <h2 className="text-sm font-bold text-slate-900 ">B2B Global Kategori Dizini</h2>
                            ...
                            <div className="flex bg-slate-100  rounded-lg p-1 border border-slate-200 ">
                                <input 
                                    ...
                                    className="bg-transparent text-[12px] font-medium px-3 outline-none text-slate-700  min-w-[200px] disabled:opacity-50" 
                                />
                                <div className="px-2 text-slate-400 font-bold border-l border-slate-200  flex items-center justify-center text-[10px]">CTRL+F</div>
                            </div>
                        </div>
                        <div className="p-6 flex-1 overflow-y-auto">
                            {!selectedLocal ? (
                                <div className="max-w-[400px] mx-auto text-center mt-12 opacity-80">
                                    ...
                                    <h3 className="text-sm font-bold text-slate-700 ">Global Dizin Kilitli 🔒</h3>
                                ...
                                <div className="space-y-4">
                                    {filteredGlobal.slice(0, 75).map((gc: any) => (
                                        <div key={gc.id} className="p-4 rounded-xl border shadow-sm transition-all flex items-center justify-between bg-white  border-indigo-200  hover:border-indigo-500 :border-indigo-400 hover:shadow-md pointer-events-none">
                                            <div className="pointer-events-auto px-2">
                                                <div className="text-[12px] font-bold text-slate-800  select-none pointer-events-none">{gc.fullName}</div>
                                    ...
                                    {filteredGlobal.length > 75 && (
                                        <div className="text-center p-4 text-[11px] font-bold tracking-widest uppercase text-slate-400 border border-dashed border-slate-200  rounded-xl">
