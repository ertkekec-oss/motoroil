...
import { getPanelHref } from "@/lib/nav/panelHref"
...
export default function StaffDealerOrdersPage() {
    ...
    async function load() {
        setLoading(true)
        setErr(null)
        const res = await fetch("/api/dealer-network/orders/approvals?status=PENDING_APPROVAL,PAID_PENDING_APPROVAL", { cache: "no-store" })
    ...
    async function openDetail(o: OrderRow) {
        ...
        setDetail(null)
        setRejectReason("")
        const res = await fetch(`/api/dealer-network/orders/${encodeURIComponent(o.id)}`, { cache: "no-store" })
        const data = await res.json().catch(() => null)
        if (!res.ok || !data?.ok) {
    ...
    async function approve(id: string) {
        ...
        const res = await fetch(`/api/dealer-network/orders/${encodeURIComponent(id)}/approve`, {
            method: "POST",
    ...
    async function reject(id: string) {
        ...
        setBusy(true); setErr(null)
        const res = await fetch(`/api/dealer-network/orders/${encodeURIComponent(id)}/reject`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            cache: "no-store",
    ...
    async function doRefund(id: string, amount: string) {
        setBusy(true); setErr(null)
        const res = await fetch(`/api/dealer-network/orders/${encodeURIComponent(id)}/refund`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            cache: "no-store",
            body: JSON.stringify({
            ...
            <div className="mx-auto w-full max-w-6xl">
                <div className="mb-6 flex items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Link
                            href={getPanelHref(currentUser?.role)}
                            className="h-10 px-4 inline-flex items-center rounded-xl border bg-card text-sm font-medium hover:bg-muted/40"
                        >
                            Panel
                        </Link>
