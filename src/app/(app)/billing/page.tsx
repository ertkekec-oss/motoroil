...
const StatCard = ({ label, value, icon: Icon, color = "indigo" }: any) => (
    <div className="bg-white/60 backdrop-blur-md border border-white p-5 rounded-2xl flex items-center gap-4 transition-all hover:shadow-lg hover:shadow-indigo-500/5 group">
        <div className={`w-12 h-12 rounded-xl bg-gradient-to-br from-${color}-500/10 to-${color}-500/20 flex items-center justify-center`}>
...
function BillingContent() {
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto font-sans bg-gradient-to-br from-indigo-50/50 via-white to-sky-50/50 min-h-screen animate-in fade-in duration-1000">
            {/* Header section with refined light aesthetic */}
            <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6 bg-white/40 backdrop-blur-xl border border-white p-8 rounded-[2.5rem] shadow-xl shadow-indigo-500/5">
                <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                ...
                <StatCard
                    ...
                    color="amber"
                />
                <div className="bg-white/60 backdrop-blur-md border border-white p-5 rounded-2xl flex flex-col justify-center gap-2 transition-all hover:shadow-lg hover:shadow-indigo-500/5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {plans.map((plan: any) => {
                    ...
                    return (
                        <div
                            key={plan.id}
                            className={`relative bg-white/70 backdrop-blur-xl border-2 rounded-[3.5rem] p-10 transition-all duration-500 group flex flex-col ${isCurrent
                                    ? 'border-emerald-400/50 shadow-2xl shadow-emerald-500/5'
                                    : isRecommended
                                        ? 'border-indigo-400/50 shadow-2xl shadow-indigo-500/10 scale-[1.03]'
                                        : 'border-white hover:border-indigo-200 hover:shadow-2xl hover:shadow-slate-500/5'
                                }`}
                        >
                            {isRecommended && (
