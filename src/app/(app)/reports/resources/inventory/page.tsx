        ...
        <div className="min-h-screen bg-slate-50 dark:bg-[#0B1220] p-4 sm:p-6 lg:p-10 pb-24 animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
                ...
                <div className="bg-white dark:bg-[#0f172a] border border-red-500/20 dark:border-red-500/10 rounded-3xl p-6 shadow-sm flex flex-col justify-between relative overflow-hidden">
                    ...
                    <div className="text-3xl font-black text-red-600 dark:text-red-400 relative z-10">₺{(idleValue / 1000).toFixed(1)}k</div>
                    <div className="text-xs font-semibold text-red-500/70 mt-2 relative z-10">Satılmayan ürünlere bağlı bütçe</div>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody>
                            {INVENTORY_DATA.map((row) => {
                                let statusBadge;
                                if (row.status === 'IDLE') statusBadge = <span className="px-2 py-1 bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-400 text-[10px] font-bold uppercase rounded border border-red-200 dark:border-red-500/20">ATIL (RİSKLİ)</span>;
                                else if (row.status === 'WARNING') statusBadge = <span className="px-2 py-1 bg-amber-100 dark:bg-amber-500/20 text-amber-600 dark:text-amber-400 text-[10px] font-bold uppercase rounded border border-amber-200 dark:border-amber-500/20">DİKKAT EDİLMELİ</span>;
                                else if (row.status === 'FAST_MOVING') statusBadge = <span className="px-2 py-1 bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 text-[10px] font-bold uppercase rounded border border-blue-200 dark:border-blue-500/20">HIZLI (YILDIZ)</span>;
