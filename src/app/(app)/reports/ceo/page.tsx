...
const InfoTooltip = ({ content, iconClassName = "w-4 h-4 text-slate-400 hover:text-slate-600" }: { content: string, iconClassName?: string }) => (
    <div className="group relative inline-flex items-center justify-center pointer-events-auto align-middle ml-1">
        ...
        <div className="opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-4 bg-white border border-slate-200 rounded-xl shadow-md text-slate-600 text-sm leading-relaxed z-[100] pointer-events-none font-normal text-left">
            {content}
...
export default function CEODashboardPage() {
        ...
        return (
            <div className="min-h-screen bg-slate-50 flex items-center justify-center text-slate-800 font-sans">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <div className="font-bold text-sm tracking-widest text-slate-500 uppercase animate-pulse">İş Zekası Yükleniyor...</div>
                </div>
            </div>
        );
    }

    if (!data) return <div className="text-slate-800 p-10 font-sans font-medium">Veri alınamadı.</div>;
    ...
    return (
        <div className="min-h-screen bg-slate-50 text-slate-800 font-sans p-6 md:p-10 pb-24 animate-in fade-in duration-500">
            ...
            <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-3xl p-8 mb-6 shadow-md relative z-10">
                <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
                    ...
                    <div className="flex flex-col gap-4 shrink-0 w-full md:w-auto">
                        ...
                        {/* Role Toggle */}
                        <div className="flex bg-white rounded-xl p-1 shadow-lg items-center self-end w-full md:w-auto">
            ...
            <div className="mb-10">
                <div className="flex items-center gap-2 mb-4">
                    ...
                    <h2 className="text-sm font-bold text-slate-800 uppercase tracking-widest">Analitik Çekirdek (Reel Metrikler)</h2>
                    <InfoTooltip content="İşletmenin anlık reel performansı." iconClassName="w-4 h-4 text-slate-400" />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                    {/* EN KÂRLI ÜRÜN */}
                    <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200 group">
                        ...
                        {metrics.mostProfitable ? (
                            <>
                                <div className="text-base font-black text-slate-900 truncate" title={metrics.mostProfitable.name}>{metrics.mostProfitable.name}</div>
                                <div className="flex items-end gap-1 mb-2">
                                    <span className="text-2xl font-black text-slate-800">+{Math.round(metrics.mostProfitable.roi)}%</span>
                                    <span className="text-xs text-emerald-600 font-bold mb-1">ROI</span>
                                </div>
                                <div className="text-xs font-medium text-slate-500 bg-slate-50 border border-slate-100 px-3 py-1.5 rounded-lg inline-flex items-center gap-2">
                    ...
                    <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200 group">
                        ...
                        <div className="text-2xl font-black text-slate-900 mb-1">₺{metrics.inventoryValue?.toLocaleString()}</div>
                    ...
                    <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200 group">
                        ...
                        <div className="text-2xl font-black text-slate-900 mb-1">₺{metrics.wipValue?.toLocaleString()}</div>
                        <div className="text-[11px] font-medium text-slate-500 leading-tight mt-3">
                    ...
                    <div className="bg-gradient-to-br from-indigo-50 to-white rounded-2xl p-5 shadow-sm border border-indigo-100 group flex flex-col justify-between">
                            ...
                            {metrics.mvpCustomer ? (
                                <div className="flex justify-between items-end">
                                    <div className="flex-1 pr-2">
                                        <div className="text-[15px] font-bold text-slate-900 truncate">{metrics.mvpCustomer.name}</div>
                                        <div className="text-[10px] text-slate-500 font-medium">Satış Lideri</div>
                                    </div>
                        ...
                        <div className="mt-4 border-t border-indigo-100/60 pt-3 flex justify-between items-center">
                            <div className="text-[10px] font-bold text-slate-500 uppercase">Per Ciro: <span className="text-slate-800">₺{Math.round(metrics.revenuePerEmployee || 0).toLocaleString()}</span></div>
                ...
                {roleMode === 'CEO' && (
                    <>
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 flex flex-col justify-center">
                            <div className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 flex justify-between items-center w-full">
                        ...
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 flex flex-col justify-center">
                            ...
                            <div className="text-3xl font-black text-slate-900">42 Gün</div>
                        </div>
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 flex flex-col justify-center">
                            <div className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 flex justify-between items-center w-full">
                ...
                {roleMode === 'CFO' && (
                    <>
                        ...
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 flex flex-col justify-center">
                            ...
                            <div className="text-3xl font-black text-slate-900">$0 <span className="text-sm font-medium text-slate-400 ml-1">(Yok)</span></div>
                        </div>
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 flex flex-col justify-center">
                            <div className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 flex justify-between items-center w-full">
                ...
                {roleMode === 'COO' && (
                    <>
                        ...
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 flex flex-col justify-center">
                            ...
                            <div className="text-3xl font-black text-slate-900">₺184K</div>
                        </div>
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 flex flex-col justify-center">
                            <div className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 flex justify-between items-center w-full">
                            ...
                            <div className="text-3xl font-black text-slate-900">14 Gün</div>
                        </div>
                    </>
                )}
                ...
                {roleMode === 'Growth' && (
                    <>
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 flex flex-col justify-center">
                            ...
                            <div className="text-xl font-black text-slate-900 leading-tight">Maden Yağları</div>
                        </div>
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 flex flex-col justify-center">
                            <div className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 flex justify-between items-center w-full">
                            ...
                            <div className="text-xl font-black text-slate-900 leading-tight">10W-40 Pro</div>
                        </div>
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 flex flex-col justify-center">
                            <div className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 flex justify-between items-center w-full">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                ...
                <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col min-h-[420px]">
                    <div className="flex justify-between items-start mb-2">
                        <div>
                            <h2 className="text-base font-bold text-slate-900 flex items-center mb-1">
                                Risk Haritası
                                <InfoTooltip content="Operasyonel risk (X) ve finansal risk (Y) yoğunluğunu gösterir." />
                            </h2>
                            <p className="text-xs text-slate-500 font-medium tracking-wide">Kırmızı: Kritik | Turuncu: Risk | Yeşil: Stabil</p>
                        </div>
                    </div>
                ...
                <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col min-h-[420px]">
                    ...
                    <div className="space-y-4 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                        {decisionQueue.map((item) => (
                            <div key={item.id} className="flex flex-col p-4 rounded-xl border border-slate-200 hover:border-slate-300 hover:shadow-md transition-all bg-white relative">
                                <div className="mb-3">
                                    <div className="text-sm font-black text-slate-900 mb-1 leading-tight">{item.action}</div>
                                    <div className="text-xs font-medium text-slate-500">{item.reason}</div>
                                </div>
                                ...
                                <div className="flex gap-2 w-full mt-auto">
                                    ...
                                    <button className="px-4 py-2 bg-slate-100 text-slate-700 rounded-lg text-xs font-bold hover:bg-slate-200 transition-colors border border-slate-200">
                                        Onayla
                                    </button>
                                    <button className="px-4 py-2 text-slate-500 hover:bg-slate-50 rounded-lg text-xs font-semibold transition-colors">
                                        Sonra
                                    </button>
                                </div>
                            </div>
