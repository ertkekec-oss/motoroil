        ...
        <div className="min-h-screen bg-slate-50 dark:bg-[#0B1220] p-4 sm:p-6 lg:p-10 pb-24 animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
                ...
                <div className="bg-white dark:bg-[#0f172a] border border-red-500/20 dark:border-red-500/10 rounded-3xl p-6 shadow-sm flex flex-col justify-between relative overflow-hidden">
                    ...
                    <div className="text-3xl font-black text-red-600 dark:text-red-400 relative z-10">₺{(totalOver90 / 1000).toLocaleString()}k</div>
                    <div className="text-xs font-semibold text-red-500/70 mt-2 relative z-10">Çok yüksek riskli kapital</div>
                </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody>
                            {AGING_DATA.map((row) => {
                                let statusBadge;
                                if (row.status === 'LEGAL') statusBadge = <span className="px-2 py-1 bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-400 text-[10px] font-bold uppercase rounded border border-red-200 dark:border-red-500/20">HUKUKİ SÜREÇ</span>;
                                else if (row.status === 'CRITICAL') statusBadge = <span className="px-2 py-1 bg-orange-100 dark:bg-orange-500/20 text-orange-600 dark:text-orange-400 text-[10px] font-bold uppercase rounded border border-orange-200 dark:border-orange-500/20">KRİTİK RİSK</span>;
                                else if (row.status === 'WARNING') statusBadge = <span className="px-2 py-1 bg-amber-100 dark:bg-amber-500/20 text-amber-600 dark:text-amber-400 text-[10px] font-bold uppercase rounded border border-amber-200 dark:border-amber-500/20">GECİKMELİ</span>;
                                else statusBadge = <span className="px-2 py-1 bg-emerald-100 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-[10px] font-bold uppercase rounded border border-emerald-200 dark:border-emerald-500/20">SAĞLIKLI</span>;
