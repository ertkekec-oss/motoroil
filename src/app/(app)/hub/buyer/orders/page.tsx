...
function StatusBadge({ status }: { status: string }) {
    const statusMap: Record<string, { label: string, colorClass: string }> = {
        ...
        CANCELLED: { label: "İptal Edildi", colorClass: "bg-slate-100 dark:bg-white/5 dark:bg-white dark:bg-[#080911]/5 text-slate-600 dark:text-slate-300 dark:text-slate-300" },
        RETURNED: { label: "İade", colorClass: "bg-slate-100 dark:bg-white/5 dark:bg-white dark:bg-[#080911]/5 text-slate-600 dark:text-slate-300 dark:text-slate-300" },
    };

    const s = statusMap[status] || { label: status, colorClass: "bg-slate-100 dark:bg-white/5 dark:bg-white dark:bg-[#080911]/5 text-slate-600 dark:text-slate-300 dark:text-slate-300" };
    ...
    return (
        <div className="bg-slate-50 dark:bg-[#080911] dark:bg-transparent min-h-screen pb-16 w-full font-sans">
            <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-in fade-in duration-300">
                ...
                {/* Ana Tablo Konteyner */}
                <div className="bg-white dark:bg-[#080911] border border-slate-200 dark:border-white/10 dark:border-white/10 rounded-2xl shadow-sm overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="w-full text-left table-auto">
                            <thead className="bg-slate-50 dark:bg-[#080911]/50 border-b border-slate-100 dark:border-white/5 dark:border-white/5 text-xs uppercase text-slate-500 dark:text-slate-400 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 font-semibold tracking-wide">
                                <tr>
                                ...
                                {data.length === 0 ? (
                                    <tr>
                                        <td colSpan={6} className="px-6 py-16 text-center">
                                            <div className="w-16 h-16 bg-slate-50 dark:bg-[#080911] rounded-2xl flex items-center justify-center text-3xl mx-auto mb-4 border border-slate-200 dark:border-white/10 dark:border-white/10 shadow-sm">
                                                🛒
                                            </div>
                    ...
                    <div className="px-6 py-4 border-t border-slate-100 dark:border-white/5 dark:border-white/5 bg-slate-50 dark:bg-[#080911]/50 flex flex-col sm:flex-row items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                            {cursor ? (
                                <Link
                                    ...
                                    className="h-9 px-4 inline-flex items-center justify-center rounded-lg text-[13px] font-semibold border border-slate-300 dark:border-white/20 dark:border-white/20 bg-white dark:bg-[#080911] text-slate-700 dark:text-slate-200 dark:text-slate-200 hover:bg-slate-50 dark:bg-[#080911] transition-colors shadow-sm"
                                >
                                    Baştan
                                </Link>
                            ) : (
                                <span className="h-9 px-4 inline-flex items-center justify-center rounded-lg text-[13px] font-semibold border border-slate-200 dark:border-white/10 dark:border-white/10 bg-slate-50 dark:bg-[#080911] text-slate-400 dark:text-slate-500 dark:text-slate-500 dark:text-slate-400 cursor-not-allowed">
                                    Baştan
                                </span>
                            )}
