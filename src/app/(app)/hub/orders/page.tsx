        ...
        <div className="bg-slate-50 dark:bg-[#0f172a] min-h-screen pb-16 w-full font-sans">
            <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-in fade-in duration-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    {/* RFQ Analytics */}
                    <Link href="/seller/rfqs" className="bg-white dark:bg-[#1e293b] px-5 py-4 rounded-xl border border-slate-200 dark:border-white/5 shadow-sm flex items-center gap-4 hover:border-indigo-500/30 transition-colors group">
                        <div className="w-10 h-10 bg-amber-50 dark:bg-amber-500/10 rounded-lg flex items-center justify-center text-amber-600 dark:text-amber-400 shrink-0 group-hover:bg-amber-100 dark:group-hover:bg-amber-500/20 transition-colors">
                            <Inbox className="w-5 h-5" />
                        </div>
                        <div className="flex-1">
                            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Cevap Bekleyen Talepler</div>
                            <div className="text-lg font-black text-slate-900 dark:text-white leading-none">{pendingRfqs} <span className="text-xs font-semibold text-slate-500 ml-1">Kabul Edilen: {acceptedRfqs}</span></div>
                        </div>
                    </Link>

                    {/* Sales Orders */}
                    <Link href="/hub/seller/orders" className="bg-emerald-50/50 dark:bg-emerald-500/5 px-5 py-4 rounded-xl border border-emerald-100 dark:border-emerald-500/10 shadow-sm flex items-center gap-4 hover:border-emerald-500/30 transition-colors group">
                        <div className="w-10 h-10 bg-emerald-100 dark:bg-emerald-500/20 rounded-lg flex items-center justify-center text-emerald-600 dark:text-emerald-400 shrink-0 group-hover:bg-emerald-200 dark:group-hover:bg-emerald-500/30 transition-colors">
                            <ShoppingCart className="w-5 h-5" />
                        </div>
                        <div className="flex-1">
                            <div className="text-[10px] font-bold text-emerald-600/70 dark:text-emerald-400/70 uppercase tracking-widest mb-0.5">Alınan Siparişler (Yeni)</div>
                            <div className="text-lg font-black text-emerald-700 dark:text-emerald-400 leading-none">{newSalesOrders} <span className="text-xs font-semibold text-emerald-600/70 ml-1">Hazırlanan: {shippedSalesOrders}</span></div>
                        </div>
                    </Link>

                    {/* Purchase Orders */}
                    <Link href="/hub/buyer/orders" className="bg-white dark:bg-[#1e293b] px-5 py-4 rounded-xl border border-slate-200 dark:border-white/5 shadow-sm flex items-center gap-4 hover:border-blue-500/30 transition-colors group">
                        <div className="w-10 h-10 bg-blue-50 dark:bg-blue-500/10 rounded-lg flex items-center justify-center text-blue-600 dark:text-blue-400 shrink-0 group-hover:bg-blue-100 dark:group-hover:bg-blue-500/20 transition-colors">
                            <ShoppingBag className="w-5 h-5" />
                        </div>
                        <div className="flex-1">
                            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Açık Siparişler (İşlemde)</div>
                            <div className="text-lg font-black text-slate-900 dark:text-white leading-none">{newPurchaseOrders} <span className="text-xs font-semibold text-slate-500 ml-1">Onaylanan: {paidPurchaseOrders}</span></div>
                        </div>
                    </Link>
                </div>
