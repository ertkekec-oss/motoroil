...
export default function PayoutsPage() {
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">💳 Para Çekme (Payouts)</h1>
            <FinanceStatusBanner />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-white dark:bg-[#0f172a] p-5 rounded-xl border border-slate-200 dark:border-white/5 shadow-sm opacity-80">
                    <p className="text-sm font-semibold text-slate-500 dark:text-slate-400 mb-1">Rezerve Edilmiş (Reserved)</p>
                    <p className="text-2xl font-bold text-slate-700">{formatMoney(balances?.reservedBalance || 0)}</p>
                    <div className="text-xs text-slate-500 dark:text-slate-400 mt-2 font-medium bg-slate-100 inline-block px-2 py-1 rounded">İhtilaf / İade teminatı</div>
                </div>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Destinations */}
                <div className="lg:col-span-1 border border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-[#1e293b] rounded-xl p-5 shadow-sm">
                        ...
                        <div className="space-y-3">
                            {destinations.map(d => (
                                <div key={d.id} className="bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-white/5 rounded-lg p-4 shadow-sm flex flex-col group hover:border-blue-300 transition-colors">
                                    ...
                                    <span className="font-mono text-xs text-slate-500 dark:text-slate-400 tracking-wider bg-slate-100 p-1.5 rounded">{d.ibanMasked}</span>
                ...
                {/* Requests */}
                <div className="lg:col-span-2 border border-slate-200 dark:border-white/5 bg-white dark:bg-[#0f172a] rounded-xl shadow-sm overflow-hidden">
                    <div className="px-6 py-5 border-b border-slate-200 dark:border-white/5 flex items-center justify-between bg-slate-50 dark:bg-[#1e293b]">
                        <h2 className="text-lg font-bold text-slate-800">Çekim Taleplerim</h2>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-left text-sm text-slate-600 dark:text-slate-400">
                            <thead className="bg-white dark:bg-[#0f172a] text-slate-400 border-b border-slate-100 text-xs uppercase tracking-wider">
                                <tr>
                                ...
                                {loading ? (
                                    ...
                                    requests.map(req => (
                                        <tr key={req.id} className="hover:bg-slate-50 dark:bg-[#1e293b] transition-colors">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-slate-500 dark:text-slate-400">{req.destination}</td>
