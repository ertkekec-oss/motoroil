...
export default function BomsPage() {
  ...
  const inputClass = isLight
    ? "w-full h-[44px] px-3 rounded-[24px] text-[13px] font-medium border border-slate-200 bg-slate-50 text-slate-800 focus:border-blue-500 outline-none transition-all"
    : "w-full h-[44px] px-3 rounded-[24px] text-[13px] font-medium border border-slate-800 bg-slate-900/50 text-slate-200 focus:border-blue-500 outline-none transition-all";
    ...
    <div data-pos-theme={theme} className={`w-full min-h-[100vh] px-8 py-8 space-y-6 transition-colors duration-300 font-sans ${isLight ? "bg-[#FAFAFA]" : ""}`}>
        ...
        <div className="flex gap-3">
          {canManage && (
            <button
              ...
              className={`h-[40px] px-5 flex items-center gap-2 rounded-full font-black text-[10px] uppercase tracking-widest transition-all shadow-sm ${isLight ? "bg-blue-600 text-white hover:bg-blue-700" : "bg-blue-600 text-white hover:bg-blue-500"}`}
            >
      ...
      <div className="flex items-center gap-4">
        <div className="relative flex-1 max-w-[500px]">
          ...
          <input
            ...
            className={`w-full h-[40px] pl-[38px] pr-4 rounded-[24px] text-[13px] font-medium border outline-none transition-all ${isLight ? "bg-white border-slate-200 text-slate-800" : "bg-slate-900 border-slate-800 text-slate-200"}`}
          />
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredBoms.map((bom) => (
            <div key={bom.id} className={`rounded-[24px] p-5 flex flex-col gap-4 border transition-all ${cardClass} hover:shadow-sm ${isLight ? 'hover:border-blue-200' : 'hover:border-blue-800'}`}>
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-[24px] flex items-center justify-center text-[15px] font-bold shrink-0 ${isLight ? "bg-blue-50 text-blue-600" : "bg-blue-900/30 text-blue-400"}`}>
              ...
              <div className={`flex items-center justify-between p-3 rounded-[24px] border ${isLight ? "bg-slate-50 border-slate-100" : "bg-slate-800/50 border-slate-800/80"}`}>
                <div>
              ...
              <div className={`flex items-center gap-2 mt-auto pt-4 border-t ${isLight ? "border-slate-100" : "border-slate-800/50"}`}>
                <button
                  ...
                  className={`flex-1 h-[36px] flex items-center justify-center gap-2 rounded-full text-[10px] font-black uppercase tracking-widest border transition-colors ${isLight ? "bg-white border-slate-200 text-slate-700 hover:bg-slate-50" : "bg-slate-900 border-slate-800 text-slate-300 hover:bg-slate-800"}`}
                >
                ...
                {canManage && (
                  <>
                    <button
                      ...
                      className={`h-[36px] px-3 rounded-full border flex items-center justify-center transition-colors ${isLight ? "bg-slate-50 border-slate-200 text-slate-600 hover:bg-slate-100" : "bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700"}`}
                    >
                      <Pencil className="w-4 h-4" />
                    </button>
                    <button
                      onClick={() => handleDeleteBom(bom.id, bom.name || bom.code)}
                      className={`h-[36px] px-3 rounded-full border flex items-center justify-center transition-colors ${isLight ? "bg-red-50 border-red-200 text-red-600 hover:bg-red-100" : "bg-red-900/20 border-red-800/50 text-red-400 hover:bg-red-900/40"}`}
                    >
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50">
          <div className={`w-[800px] max-w-full rounded-[24px] overflow-hidden flex flex-col max-h-[90vh] ${modalClass} animate-in zoom-in-95 duration-200`}>
            ...
            <div className="p-6 overflow-y-auto custom-scroll flex flex-col gap-6">
              {/* Parent Info */}
              <div className={`p-4 rounded-[24px] border ${isLight ? "bg-slate-50 border-slate-200" : "bg-slate-800/30 border-slate-700"}`}>
              ...
              <div>
                ...
                {newBom.items.length === 0 ? (
                  <div className={`py-8 text-center rounded-[24px] border border-dashed ${isLight ? "bg-slate-50 border-slate-300" : "bg-slate-900/50 border-slate-700"}`}>
                  ...
                  <div className="space-y-3">
                    {newBom.items.map((item, idx) => (
                      <div key={idx} className={`flex items-start gap-3 p-3 rounded-[24px] border ${isLight ? "bg-white border-slate-200" : "bg-slate-800 border-slate-700"}`}>
                        <div className="flex-1">
                          ...
                          <select
                            ...
                            className={`w-full h-[36px] px-2 rounded-full text-[12px] border outline-none ${isLight ? "bg-slate-50 border-slate-200" : "bg-slate-900 border-slate-700 text-slate-200"}`}
                          >
                        ...
                        <div className="w-[80px]">
                          ...
                          <input
                            ...
                            value={item.unit}
                            disabled
                            className={`w-full h-[36px] px-2 rounded-full text-[12px] border outline-none opacity-70 ${isLight ? "bg-slate-100 border-slate-200 text-slate-800" : "bg-slate-900 border-slate-700 text-slate-400"}`}
                          />
                        </div>
                        ...
                        <div className="pt-[18px]">
                          <button
                            ...
                            className={`w-[36px] h-[36px] flex items-center justify-center rounded-full border transition-colors ${isLight ? "text-red-500 border-red-100 hover:bg-red-50" : "text-red-400 border-red-900/50 hover:bg-red-900/20"}`}
                          >
                            <Trash2 className="w-4 h-4" />
                          </button>
                        </div>
            ...
            <div className={`p-6 bg-slate-50 dark:bg-slate-900 border-t ${isLight ? "border-slate-100" : "border-slate-800"}`}>
              <button 
                ...
                className={`w-full h-[44px] rounded-full text-[10px] font-black uppercase tracking-widest text-white transition-all flex justify-center items-center gap-2 ${isLight ? "bg-blue-600 hover:bg-blue-700" : "bg-blue-600 hover:bg-blue-500"} ${isProcessing ? 'opacity-50' : ''}`}
              >
                {isProcessing ? "Kaydediliyor..." : "REÇETEYİ KAYDET VE KAPAT"}
            ...
            <div className="p-6 overflow-y-auto custom-scroll flex flex-col gap-4">
              ...
              <div className="space-y-2">
                {selectedBom.items?.map((item: any, idx: number) => (
                  <div key={idx} className={`flex justify-between items-center p-3 rounded-full border ${isLight ? "bg-white border-slate-100" : "bg-slate-800/50 border-slate-700"}`}>
                    <div>
