...
export async function POST(request: Request) {
    ...
    let debugStep = 'INIT_AND_BODY_PARSE';
    const timeoutPromise = new Promise<any>((_, reject) => setTimeout(() => reject(new Error('TRACER_TIMEOUT: Sistem bu adımda tıkandı -> ' + debugStep)), 8000));
    ...
    const executor = (async () => {
        ...
        debugStep = 'PRISMA_TRANSACTION: Enrich Items';
        const result = await prisma.$transaction(async (tx) => {
            ...
            debugStep = 'PRISMA_TRANSACTION: Campaign Evaluation';
            ...
            debugStep = 'PRISMA_TRANSACTION: Create Order';
            ...
            debugStep = 'PRISMA_TRANSACTION: Update Stocks';
            ...
            debugStep = 'PRISMA_TRANSACTION: Update Kasa';
            // C. Update Kasa
        ...
        debugStep = 'OUTSIDE_TX: Bank Commission';
        ...
        debugStep = 'OUTSIDE_TX: Accounting Ledger (Journal)';
        // --- ACCOUNTING FALLBACK ---
    ...
    try {
        const finalResponse = await Promise.race([executor, timeoutPromise]);
