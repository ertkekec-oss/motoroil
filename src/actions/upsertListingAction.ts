...
export async function upsertListingAction(formData: FormData) {
    try {
        ...
        if (!user) {
            return { success: false, error: "Oturum süreniz dolmuş veya yetkisizsiniz." };
        }

        const role = user.role?.toUpperCase?.() || "";
        const isAdmin = role === "SUPER_ADMIN" || role === "ADMIN" || role === "PLATFORM_ADMIN" || role === "OWNER";
        ...
        if (!erpProductId || !priceStr || !availableQtyStr || !status) {
            return { success: false, error: "Eksik veya hatalı bilgi girdiniz. Lütfen zorunlu alanları doldurun." };
        }
        ...
        if (!erpProduct) {
            return { success: false, error: "Ürün ERP sisteminde bulunamadı." };
        }

        if (!companyId) {
            if (isAdmin || user.tenantId === erpProduct.tenantId) {
                companyId = erpProduct.companyId;
            } else {
                return { success: false, error: "Firma (Company) kimliği bulunamadı, ürün yayınlanamaz." };
            }
        }

        if (erpProduct.companyId !== companyId && !isAdmin) {
            return { success: false, error: "Bu ürünü yayınlamaya yetkiniz yok." };
        }
        ...
        return { success: false, error: error.message || "Beklenmeyen bir sunucu hatası oluştu." };
    }
}
