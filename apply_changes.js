...
const oldFilter = \    const filteredHistory = historyList.filter(item => {
        if (item.type === 'Vadelendirme') return false;
        if (activeTab === 'all') return true;
        if (activeTab === 'sales') return item.type === 'Fatura' || item.type === 'Satýþ' || item.type === 'Ýrsaliye';
...
const newFilter = \    const filteredHistory = historyList.filter(item => {
        ...
        else if (activeTab === 'sales') matchesTab = (item.type === 'Fatura' || item.type === 'Satýþ' || item.type === 'Ýrsaliye');
...
data = data.replace(oldFilter, newFilter);
...
const newTableStart = \                    ) : (
                        <div className="flex flex-col">
                            ...
                            <div className="p-4 md:px-6 md:py-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-[var(--border-color,rgba(255,255,255,0.05))]">
                                <h3 className="text-[13px] font-black uppercase tracking-widest text-[#1e293b] dark:text-white">Kayýt Listesi</h3>
...
data = data.replace(oldTableStart, newTableStart);
...
data = data.replace(oldTableEnd, newTableEnd);
